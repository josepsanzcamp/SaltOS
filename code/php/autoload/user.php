    ...
    if (isset($_USER["id"])) {
        ...
        if (db_num_rows($result) == 1) {
            ...
            while ($row2 = db_fetch_row($result2)) {
                ...
                if (!isset($_USER[$aplicacion])) {
                    $_USER[$aplicacion] = array();
                }
                $_USER[$aplicacion][$permiso] = (
                    (isset($_USER[$aplicacion][$permiso]) ? $_USER[$aplicacion][$permiso] : 1) &&
                    $row2["allow"] &&
                    !$row2["deny"]
                ) ? 1 : 0;
            }
...
function check_time()
{
    ...
    $dia = (date("w", time()) + 6) % 7;
    $check_hora1 = (
        $_USER["hora_ini"] <= $_USER["hora_fin"] &&
        ($hora < $_USER["hora_ini"] || $hora > $_USER["hora_fin"])
    );
    $check_hora2 = (
        $_USER["hora_ini"] > $_USER["hora_fin"] &&
        $hora < $_USER["hora_ini"] &&
...
function check_user($aplicacion = "", $permiso = "")
{
    ...
    if (!isset($_USER[$aplicacion][$permiso])) {
        return 0;
    }
...
function check_basicauth()
{
    ...
    if (!getServer("PHP_AUTH_USER")) {
        return;
    }
    if (!getServer("PHP_AUTH_PW")) {
        return;
    }
    if (!check_security("retries") && !check_security("logouts")) {
        sess_close();
...
{
    if (!eval_bool(getDefault("security/filterenabled"))) {
        return true;
    }
...
function check_captcha($captcha = "")
{
    ...
    if (strlen($captcha1) == 0) {
        $valid = 0;
    }
    if (strlen($captcha2) == 0) {
        $valid = 0;
    }
    if ($captcha1 == "null") {
        $valid = 0;
