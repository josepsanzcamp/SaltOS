...
function solve_style($style)
{
    ...
    if (isset($temp1[2])) {
        foreach ($_CONFIG["styles2"] as $style2) {
            $temp2 = explode("/", $style2["value"]);
            if (isset($temp2[2])) {
                if ($temp1[0] == $temp2[0] && $temp1[2] == $temp2[2]) {
                    return $style2["value"];
                }
            }
        }
...
{
    if (stripos($style, "light") !== false) {
        return "light";
    }
    if (stripos($style, "dark") !== false) {
        return "dark";
    }
    return "unknown";
}
...
function get_css_files()
{
    ...
    if (load_style($style)) {
        $result[] = $stylepre . $style . $stylepost;
    }
    // TRICK FOR JSTREE
    ...
    if (load_style($style)) {
        $result[] = $jstreepre . $jstree . $jstreepost;
    }
    // CONTINUE
    return $result;
}
