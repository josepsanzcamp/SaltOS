...
function unoconv2pdf($input)
{
    ...
    if (!file_exists($output)) {
        ...
        if ($type == "application/pdf") {
            ...
            (in_array($ext, __unoconv_list()) && !in_array($type0, array("audio","video"))) ||
            in_array($type0, array("text","message","image"))
        ) {
...
function unoconv2txt($input)
{
    ...
    if (!file_exists($output)) {
        ...
        if (in_array($type, array("text/plain","application/json"))) {
            ...
            (in_array($ext, __unoconv_list()) && !in_array($type0, array("image","audio","video"))) ||
            in_array($type0, array("text","message"))
        ) {
...
{
    if (!check_commands(getDefault("commands/soffice"), 60)) {
        return array();
    }
...
{
    if (!check_commands(array(getDefault("commands/convert"),getDefault("commands/tesseract")), 60)) {
        return "";
    }
...
{
    if (!check_commands(getDefault("commands/pdftoppm"), 60)) {
        return "";
    }
                                    ...
                                    if (is_array($line["value"])) {
                                        foreach ($line["value"] as $word) {
                                            $lines[] = array_merge(
                                                ...
                                                array(__unoconv_node2value($word))
                                            );
                                            $words++;
                                        }
                                    }
