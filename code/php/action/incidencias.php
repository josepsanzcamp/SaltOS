...
if ($page == "incidencias") {
    ...
    $query = "SELECT " . make_extra_query_with_field("email", "d.") . " mailto," .
        ...
        WHERE a.id='$id_incidencia'
            AND (
                FIND_IN_SET(d.id,a.ids_implicados)
                OR
                FIND_IN_SET(d.id,a.ids_asignados)
            )";
    ...
    if (db_num_rows($result2)) {
        ...
        if (db_num_rows($result2)) {
            ...
            $subject .= ": " . $row2["nombre"];
            // BODY INCIDENCIAS
            $campos = array("username","datetime","cliente","proyecto","nombre",
                "estado","prioridad","categoria","descripcion");
            ...
            if (__getmail_checkperm($id_correo)) {
                ...
                if ($decoded) {
                    ...
                    foreach ($info["emails"] as $email) {
                        if ($email["valor"] != "" && strpos($email_support, $email["valor"]) !== false) {
                            $ok = 1;
                        }
                    }
                }
