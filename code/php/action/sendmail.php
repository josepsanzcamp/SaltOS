...
if(getParam("action")=="sendmail") {
	...
	$query="SELECT a.id,a.id_cuenta,a.uidl FROM tbl_correo a LEFT JOIN tbl_registros e ON e.id_aplicacion='".page2id("correo")."' AND e.id_registro=a.id AND e.primero='1' WHERE e.id_usuario='".current_user()."' AND a.is_outbox='1' AND a.state_sent='0'";
	$result=execute_query($query);
	if(!$result) {
		if(!getParam("ajax")) {
