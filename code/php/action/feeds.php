...
if (getParam("action") == "feeds") {
        ...
        if ($ids) {
            ...
            if ($numresult == $numids) {
                ...
                if ($action2[0] == "leidos") {
                    ...
                    // MOSTRAR RESULTADO
                    session_alert(
                        LANG($action2[1] ? "msgnumnoleidos" : "msgnumsileidos", "feeds") .
                        $numids .
                        LANG("message" . min($numids, 2), "feeds")
                    );
                    ...
                    // MOSTRAR RESULTADO
                    session_alert(
                        LANG($action2[1] ? "msgnumsiwait" : "msgnumnowait", "feeds") .
                        $numids .
                        LANG("message" . min($numids, 2), "feeds")
                    );
                    ...
                    // MOSTRAR RESULTADO
                    session_alert(
                        LANG($action2[1] ? "msgnumsicool" : "msgnumnocool", "feeds") .
                        $numids .
                        LANG("message" . min($numids, 2), "feeds")
                    );
                } elseif ($action2[0] == "delete") {
                    // CREAR DATOS EN TABLA DE POSTS BORRADOS
                    $query = "INSERT INTO tbl_feeds_d(id_feed,link,datetime)
                        SELECT id_feed,link,'" . current_datetime() . "'
                        FROM tbl_feeds
                        WHERE id IN (${ids})";
                    db_query($query);
                    // BORRAR POSTS
            ...
            if ($error) {
                ...
                if (isset($tagfeed)) {
                    ...
                    if (isset($url3)) {
                        ...
                        if (!isset($array2["scheme"])) {
                            $array2["scheme"] = $array["scheme"];
                        }
                        if (!isset($array2["host"])) {
                            $array2["host"] = $array["host"];
                        }
                        if (!isset($array2["port"]) && isset($array["port"])) {
                            $array2["port"] = $array["port"];
                        }
                        if (!isset($array2["path"])) {
                            $array2["path"] = "/";
                        } elseif (substr($array2["path"], 0, 1) != "/") {
                            if (isset($array["path"])) {
                                $array2["path"] = $array["path"] . "/" . $array2["path"];
                            }
                            if (!isset($array["path"])) {
                                $array2["path"] = "/" . $array2["path"];
                            }
                        }
    ...
    if (getParam("id")) {
        ...
        if ($cid == "body") {
            ...
            foreach ($lista as $key => $val) {
                ...
                $buffer .= __HTML_TEXT_OPEN__;
                if ($val["link"] != "") {
                    $buffer .= "<a onclick='parent.openwin(this.href);return false' " .
                        "href='" . addslashes($row[$val["link"]]) . "'>";
                }
                $buffer .= "<b>" . $row[$key] . "</b>";
                if ($val["link"] != "") {
                    $buffer .= "</a>";
    ...
    if (eval_bool(getDefault("debug/cancelfeeds"))) {
        die();
    }
    // CHECK THE SEMAPHORE
                    ...
                    if (count($array)) {
                        ...
                        foreach ($array as $item) {
                            ...
                            if (!in_array($link, array_merge($result2, $result4))) {
                                // SI NO ESTA EL LINK DESCARGADO EN LOS LINKS EXISTENTES
                                // NI EN LOS MARCADOS COMO BORRADOS
                        ...
                        $query = "DELETE FROM tbl_feeds_d
                            WHERE id_feed='${id_feed}'
                                AND NOT link IN (${links})
                                AND UNIX_TIMESTAMP(datetime)<='${unixtime_d}'";
                        db_query($query);
                    }
                }
            }
    ...
    if (!getParam("ajax")) {
        if ($newfeeds + $modifiedfeeds > 0) {
            if ($newfeeds > 0) {
                session_alert($newfeeds . LANG("msgnewfeedsok" . min($newfeeds, 2), "feeds"));
            }
            if ($modifiedfeeds > 0) {
                session_alert($modifiedfeeds . LANG("msgmodifiedfeedsok" . min($modifiedfeeds, 2), "feeds"));
            }
        } else {
        ...
        if ($newfeeds + $modifiedfeeds > 0) {
            $gotofeeds = " [<a href='javascript:void(0)' onclick='gotofeeds()'>" .
                LANG("msggotofeeds", "feeds") .
                "</a>]";
