...
if (getParam("action") == "pagerlist") {
    ...
    if ($minpage > 1) {
        set_array($_RESULT["rows"], "row", array("label" => 1,"value" => 1));
    }
    for ($i = $minpage; $i <= $maxpage; $i++) {
        set_array($_RESULT["rows"], "row", array("label" => $i,"value" => $i));
    }
    if ($maxpage < $totalpages) {
        set_array($_RESULT["rows"], "row", array("label" => $totalpages,"value" => $totalpages));
    }
    // PREPARE VALUE VAR
    ...
    // PREPARE LAST STRING
    $_RESULT["info"] = LANG("paginaspc") . $currentpage . LANG("spcdespc") . $totalpages .
        " (" . LANG("regsfrom") . " " . $currentregini . LANG("spcalspc") . $currentregend .
            LANG("spcdespc") . $result["count"] . ").";
    // PREPARE THE OUTPUT
