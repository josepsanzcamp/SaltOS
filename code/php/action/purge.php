...
if (!eval_bool(getDefault("enablepurge"))) {
    die();
}
// CHECK THE SEMAPHORE
...
foreach ($rows as $row) {
    ...
    if ($row["id_aplicacion"]) {
        if (!is_numeric($row["id_aplicacion"]) && !page2id($row["id_aplicacion"])) {
            show_php_error(array("phperror" => "id_aplicacion " . $row["id_aplicacion"] . " not found"));
        }
        if (is_numeric($row["id_aplicacion"]) && !id2page($row["id_aplicacion"])) {
            show_php_error(array("phperror" => "id_aplicacion " . $row["id_aplicacion"] . " not found"));
        }
        if (!is_numeric($row["id_aplicacion"])) {
            $row["id_aplicacion"] = page2id($row["id_aplicacion"]);
        }
    }
    if ($row["id_usuario"]) {
        if (!is_numeric($row["id_usuario"]) && !user2id($row["id_usuario"])) {
            show_php_error(array("phperror" => "id_usuario " . $row["id_usuario"] . " not found"));
        }
        if (is_numeric($row["id_usuario"]) && !id2user($row["id_usuario"])) {
            show_php_error(array("phperror" => "id_usuario " . $row["id_usuario"] . " not found"));
        }
        if (!is_numeric($row["id_usuario"])) {
            $row["id_usuario"] = user2id($row["id_usuario"]);
        }
    }
    if ($row["id_cuenta"]) {
        if ($row["id_aplicacion"] != page2id("correo")) {
            show_php_error(array("phperror" => "id_cuenta only can be used for correo application"));
        }
        if (!$row["id_usuario"]) {
            show_php_error(array("phperror" => "id_cuenta need id_usuario too"));
        }
        if (!is_numeric($row["id_cuenta"])) {
            $temp = execute_query("SELECT id
                FROM tbl_usuarios_c
                WHERE id_usuario='" . $row["id_usuario"] . "'
                    AND email_from='" . $row["id_cuenta"] . "'");
            if (is_array($temp)) {
                show_php_error(array("phperror" => "id_cuenta " . $row["id_cuenta"] . " found multiples times"));
            }
            if (is_null($temp)) {
                show_php_error(array("phperror" => "id_cuenta " . $row["id_cuenta"] . " not found"));
            }
            $row["id_cuenta"] = $temp;
        }
        if (!execute_query("SELECT id FROM tbl_usuarios_c WHERE id='" . $row["id_cuenta"] . "'")) {
            show_php_error(array("phperror" => "id_cuenta " . $row["id_cuenta"] . " not found"));
        }
    }
    if ($row["id_feed"]) {
        if ($row["id_aplicacion"] != page2id("feeds")) {
            show_php_error(array("phperror" => "id_feed only can be used for feeds application"));
        }
        if (!$row["id_usuario"]) {
            show_php_error(array("phperror" => "id_feed need id_usuario too"));
        }
        if (!is_numeric($row["id_feed"])) {
            $temp = execute_query("SELECT id
                FROM tbl_usuarios_f
                WHERE id_usuario='" . $row["id_usuario"] . "'
                    AND title='" . $row["id_feed"] . "'");
            if (is_array($temp)) {
                show_php_error(array("phperror" => "id_feed " . $row["id_feed"] . " found multiples times"));
            }
            if (is_null($temp)) {
                show_php_error(array("phperror" => "id_feed " . $row["id_feed"] . " not found"));
            }
            $row["id_feed"] = $temp;
        }
        if (!execute_query("SELECT id FROM tbl_usuarios_f WHERE id='" . $row["id_feed"] . "'")) {
            show_php_error(array("phperror" => "id_feed " . $row["id_feed"] . " not found"));
        }
    }
    if (!$row["days"]) {
        show_php_error(array("phperror" => "days needed by purge action"));
    }
    ...
    for (;;) {
        ...
        foreach ($rows2 as $row2) {
            ...
            // BORRAR FICHEROS DEL SISTEMA DE FICHEROS
            if ($row2["id_aplicacion"] != page2id("correo")) {
                $query = "SELECT CONCAT('" . get_directory("dirs/filesdir") . "',fichero_file) action_delete
                    FROM tbl_ficheros
                    WHERE id_aplicacion='" . $row2["id_aplicacion"] . "'
                        AND id_registro='" . $row2["id_registro"] . "'";
                $rows3 = execute_query_array($query);
                foreach ($rows3 as $delete) {
                    if (file_exists($delete) && is_file($delete)) {
                        unlink_protected($delete);
...
foreach ($rows as $row) {
    ...
    if (!$row["table"]) {
        show_php_error(array("phperror" => "table needed by purge action"));
    }
    if (!$row["field"]) {
        show_php_error(array("phperror" => "field needed by purge action"));
    }
    if (!$row["days"]) {
        show_php_error(array("phperror" => "days needed by purge action"));
    }
...
foreach ($rows as $row) {
    ...
    if (!$row["glob"]) {
        show_php_error(array("phperror" => "glob needed by purge action"));
    }
    if (!$row["action"]) {
        show_php_error(array("phperror" => "action needed by purge action"));
    }
    if (!in_array($row["action"], array("delete","truncate"))) {
        show_php_error(array("phperror" => "action only can be delete or truncate"));
    }
    if (!$row["days"]) {
        show_php_error(array("phperror" => "days needed by purge action"));
    }
...
// SEND RESPONSE
if ($total_apps) {
    javascript_alert($total_apps . LANG("msgregisterspurged" . min($total_apps, 2)));
}
if ($total_tables) {
    javascript_alert($total_tables . LANG("msgregisterspurged" . min($total_tables, 2)));
}
if ($total_files) {
    javascript_alert($total_files . LANG("msgregisterspurged" . min($total_files, 2)));
}
// RELEASE SEMAPHORE
