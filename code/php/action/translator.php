...
if (!eval_bool(getDefault("enabletranslator"))) {
    die();
}
require_once "php/libaction.php";
// CHECK COMMANDS
if (!(check_commands(getDefault("commands/translate"), 60) || check_commands(getDefault("commands/aspell"), 60))) {
    die();
}
// GET PARAMETERS
$text = getParam("text");
...
if ($langs == "auto") {
    $cache = get_cache_file(array(
        "translator",
        "auto",
        $text,
        __translator_get_langs(),
        __translator_get_aspell_langs()
    ), ".txt");
...
if (!$error && count($langs_array) != 2) {
    $error = 1;
}
if (!$error && strlen($langs_array[0]) != 2) {
    $error = 1;
}
if (!$error && strlen($langs_array[1]) != 2) {
    $error = 1;
}
if ($error) {
    action_denied();
...
if (!file_exists($cache)) {
    ...
    if ($langs_array[0] != $langs_array[1]) {
