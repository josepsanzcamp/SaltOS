...
if ($page == "clientes") {
    ...
    foreach (array_merge($_POST, $_GET) as $key => $val) {
        if (substr($key, 0, 13) == "prefix_partes") {
            if (getParam($val . "liquidar")) {
                        $prefix[] = $val;
            }
        }
    }
...
if ($page == "proyectos" && getParam("extra") == "facturas") {
    ...
    if (!$id_factura) {
        ...
        $prefix = array();
        foreach (array_merge($_POST, $_GET) as $key => $val) {
            if (substr($key, 0, 17) == "prefix_tareas_old") {
                $prefix[] = $val;
            }
        }
        $ids = array();
        ...
        $prefix = array();
        foreach (array_merge($_POST, $_GET) as $key => $val) {
            if (substr($key, 0, 20) == "prefix_productos_old") {
                $prefix[] = $val;
            }
        }
        $ids = array();
...
if ($page == "periodicas") {
    ...
    if ($ids && $meses) {
        $query2 = "SELECT *,(SELECT nombre FROM tbl_clientes WHERE id_cliente=tbl_clientes.id) cliente
            FROM tbl_periodicas
            WHERE id IN ($ids)
            ORDER BY id ASC";
        $result = db_query($query2);
