...
if (getParam("action") == "calendar") {
    ...
    $query = "SELECT COUNT(*) count,MIN(dstart) min,MAX(dstop) max
        FROM tbl_agenda
        WHERE id IN ($ids)
            AND dstart!='0000-00-00 00:00:00'
            AND dstop!='0000-00-00 00:00:00'";
    ...
    for ($i = $valmin; $i <= $valmax; $i++) {
        ...
        set_array($_RESULT["options"], "option", array(
            "label" => $meses[$imonth] . " " . $iyear,
            "value" => $value,
            "selected" => $selected
        ));
    }
    ...
    while ($unix <= $dstop) {
        ...
        if ($current == $today) {
            $data = LANG("today", "agenda") . ", " . $data;
        }
        if ($current == $yesterday) {
            $data = LANG("yesterday", "agenda") . ", " . $data;
        }
        if ($current == $tomorrow) {
            $data = LANG("tomorrow", "agenda") . ", " . $data;
        }
    ...
    while ($row = db_fetch_row($result)) {
        $id = $row["id"];
        $subclass = "";
        if ($row["activo"] && $row["dstop"] <= $current) {
            $subclass .= " red";
        }
        if ($row["activo"] && $row["dstop"] > $current) {
            $subclass .= " blue";
        ...
        while ($unixini + $maxiter * 86400 <= $unixfin) {
            ...
            if ($current2 == $yesterday || $current2 == $today || $current2 == $tomorrow) {
                $class2 .= " opened";
            }
            $maxiter++;
        }
        ...
        while ($unixini + $curiter * 86400 <= $unixfin) {
            ...
            if ($fechaini == $fechafin) {
                ...
                if ($curiter == 0) {
                    $horario = $horaini;
                } elseif ($curiter < $maxiter - 1) {
                    $horario = LANG("continue", "agenda");
                } else {
                    $horario = $horafin;
                }
            }
            ...
            if (!$propietario && $usuario) {
                $data .= " <span title='$usuario'>$usuario2</span>";
            }
    ...
    while ($unix <= $dstop) {
        ...
        $cell = $letter . $number;
        $data = "<a href='javascript:void(0)' title=\"$create\" " .
            "onclick='opencontent(\"" .
            "?page=agenda&action=form&id=0&dstart=$current+10:00:00&dstop=$current+10:00:00" .
            "\")'>$create</a>";
