...
if (load_style($style)) {
    ...
    echo "$(document).on('mouseout',inputs,function() { $(this).removeClass('ui-state-hover'); });";
    echo "var inputs='.tabla td.tbody';";
    echo "$(document).on('mouseover',inputs,function() { $(this).parent().find('td').addClass('ui-state-highlight'); });";
    echo "$(document).on('mouseout',inputs,function() { $(this).parent().find('td').removeClass('ui-state-highlight'); });";
    echo "</script>";
    echo "</head>";
    echo "<body>";
    echo str_replace(
    ...
    if ($filter != "") {
        ...
        foreach ($styles as $index => $row) {
            ...
            foreach ($filter as $filter2) {
                if (stripos($row["label"], $filter2) !== false) {
                    $found = 1;
                }
                if (stripos($row["value"], $filter2) !== false) {
                    $found = 1;
                }
            }
            if (!$found) {
