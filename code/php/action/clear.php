...
if($page=="agenda") {
	...
	$query="SELECT a.id FROM tbl_agenda a LEFT JOIN tbl_registros_i f ON f.id_aplicacion='".page2id("agenda")."' AND f.id_registro=a.id AND LEFT JOIN tbl_estados c ON a.id_estado=c.id WHERE f.id_usuario='".current_user()."' AND activo='1' AND notify_delay!='0' AND notify_dstart='1' AND /*SQLITE STRFTIME('%s','".current_datetime()."')+0.0 > STRFTIME('%s',dstart)+notify_delay*3600.0*notify_sign AND STRFTIME('%s','".current_datetime()."') < STRFTIME('%s',dstop) *//*MYSQL UNIX_TIMESTAMP('".current_datetime()."')+0.0 > UNIX_TIMESTAMP(dstart)+notify_delay*3600.0*notify_sign AND UNIX_TIMESTAMP('".current_datetime()."') < UNIX_TIMESTAMP(dstop) */";
