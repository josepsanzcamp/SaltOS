...
{
    if (!check_commands(getDefault("commands/aspell"), 60)) {
        return array();
    }
...
{
    if (!check_commands(getDefault("commands/translate"), 60)) {
        return array();
    }
...
function __translator_detect_aspell_langs($text, $length = 50)
{
    ...
    foreach ($langs as $lang) {
        $aspell = ob_passthru(str_replace(
            array("__LANG__","__INPUT__"),
            array($lang,$input),
            getDefault("commands/__aspell__")
        ));
...
function __translator_aspell($text, $lang)
{
    ...
    foreach ($aspell as $line) {
        ...
        if ($token == "&") {
            $word = strtok(" ");
            if (strtoupper(mb_substr($word, 0, 1)) == mb_substr($word, 0, 1)) {
                continue;
            }
            $number = strtok(" ");
            $offset = strtok(": ");
...
function __translator($text, $langs)
{
    ...
    file_put_contents($input, $text);
    $langs = explode("-", $langs);
    $text = ob_passthru(str_replace(
        array("__FROM__","__TO__","__INPUT__"),
        array($langs[0],$langs[1],$input),
        getDefault("commands/__translate__")
    ));
    unlink($input);
    return $text;
}
