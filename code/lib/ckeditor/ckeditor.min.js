...
*/
(function(){if(!window.CKEDITOR||!window.CKEDITOR.dom)window.CKEDITOR||(window.CKEDITOR=function(){var a=/(^|.*[\\\/])ckeditor\.js(?:\?.*|;.*)?$/i,f={timestamp:"EAKK",version:"4.4.6 DEV",revision:"8017775",rnd:Math.floor(900*Math.random())+100,_:{pending:[],basePathSrcPattern:a},status:"unloaded",basePath:function(){var b=window.CKEDITOR_BASEPATH||"";if(!b)for(var g=document.getElementsByTagName("script"),c=0;c<g.length;c++){var d=g[c].src.match(a);if(d){b=d[1];break}}-1==b.indexOf(":/")&&"//"!=b.slice(0,
...
if(e.type==CKEDITOR.NODE_TEXT){e.split(f);e.equals(g)&&(g=e.getNext())}else if(f)if(f>=e.getChildCount()){e=e.append(a.document.createText(""));i=true}else e=e.getChild(f).getPrevious();else{e=e.append(a.document.createText(""),1);i=true}var f=e.getParents(),h=g.getParents(),w,x,v;for(w=0;w<f.length;w++){x=f[w];v=h[w];if(!x.equals(v))break}for(var s=c,y,z,B,A=w;A<f.length;A++){y=f[A];s&&!y.equals(e)&&(z=s.append(y.clone()));for(y=y.getNext();y;){if(y.equals(h[A])||y.equals(g))break;B=y.getNext();
...
h,i,j,w,x,v=false,s,y;s=this.startContainer;var z=this.startOffset;if(s.type==CKEDITOR.NODE_TEXT){if(z){s=!CKEDITOR.tools.trim(s.substring(0,z)).length&&s;v=!!s}if(s&&!(w=s.getPrevious()))j=s.getParent()}else{z&&(w=s.getChild(z-1)||s.getLast());w||(j=s)}for(j=c(j);j||w;){if(j&&!w){!x&&j.equals(g)&&(x=true);if(e?j.isBlockBoundary():!f.contains(j))break;if(!v||j.getComputedStyle("display")!="inline"){v=false;x?h=j:this.setStartBefore(j)}w=j.getPrevious()}for(;w;){s=false;if(w.type==CKEDITOR.NODE_COMMENT)w=
w.getPrevious();else{if(w.type==CKEDITOR.NODE_TEXT){y=w.getText();d.test(y)&&(w=null);s=/[\s\ufeff]$/.test(y)}else if((w.$.offsetWidth>(CKEDITOR.env.webkit?1:0)||b&&w.is("br"))&&!w.data("cke-bookmark"))if(v&&CKEDITOR.dtd.$removeEmpty[w.getName()]){y=w.getText();if(d.test(y))w=null;else for(var z=w.$.getElementsByTagName("*"),B=0,A;A=z[B++];)if(!CKEDITOR.dtd.$removeEmpty[A.nodeName.toLowerCase()]){w=null;break}w&&(s=!!y.length)}else w=null;s&&(v?x?h=j:j&&this.setStartBefore(j):v=true);if(w){s=w.getPrevious();
if(!j&&!s){j=w;w=null;break}w=s}else j=null}}j&&(j=c(j.getParent()))}s=this.endContainer;z=this.endOffset;j=w=null;x=v=false;var F=function(a,b){var c=new CKEDITOR.dom.range(f);c.setStart(a,b);c.setEndAt(f,CKEDITOR.POSITION_BEFORE_END);var c=new CKEDITOR.dom.walker(c),e;for(c.guard=function(a){return!(a.type==CKEDITOR.NODE_ELEMENT&&a.isBlockBoundary())};e=c.next();){if(e.type!=CKEDITOR.NODE_TEXT)return false;y=e!=a?e.getText():e.substring(b);if(d.test(y))return false}return true};if(s.type==CKEDITOR.NODE_TEXT)if(CKEDITOR.tools.trim(s.substring(z)).length)v=
true;else{v=!s.getLength();if(z==s.getLength()){if(!(w=s.getNext()))j=s.getParent()}else F(s,z)&&(j=s.getParent())}else(w=s.getChild(z))||(j=s);for(;j||w;){if(j&&!w){!x&&j.equals(g)&&(x=true);if(e?j.isBlockBoundary():!f.contains(j))break;if(!v||j.getComputedStyle("display")!="inline"){v=false;x?i=j:j&&this.setEndAfter(j)}w=j.getNext()}for(;w;){s=false;if(w.type==CKEDITOR.NODE_TEXT){y=w.getText();F(w,0)||(w=null);s=/^[\s\ufeff]/.test(y)}else if(w.type==CKEDITOR.NODE_ELEMENT){if((w.$.offsetWidth>0||
b&&w.is("br"))&&!w.data("cke-bookmark"))if(v&&CKEDITOR.dtd.$removeEmpty[w.getName()]){y=w.getText();if(d.test(y))w=null;else{z=w.$.getElementsByTagName("*");for(B=0;A=z[B++];)if(!CKEDITOR.dtd.$removeEmpty[A.nodeName.toLowerCase()]){w=null;break}}w&&(s=!!y.length)}else w=null}else s=1;s&&v&&(x?i=j:this.setEndAfter(j));if(w){s=w.getNext();if(!j&&!s){j=w;w=null;break}w=s}else j=null}j&&(j=c(j.getParent()))}if(h&&i){g=h.contains(i)?i:h;this.setStartBefore(g);this.setEndAfter(g)}break;case CKEDITOR.ENLARGE_BLOCK_CONTENTS:case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:j=
...
e.getLength())d.setStartAfter(e);else{d.setStartBefore(e);j=0}else d.setStartBefore(e);if(g&&g.type==CKEDITOR.NODE_TEXT)if(h)if(h>=g.getLength())d.setEndAfter(g);else{d.setEndAfter(g);i=0}else d.setEndBefore(g);var d=new CKEDITOR.dom.walker(d),w=CKEDITOR.dom.walker.bookmark();d.evaluator=function(b){return b.type==(a==CKEDITOR.SHRINK_ELEMENT?CKEDITOR.NODE_ELEMENT:CKEDITOR.NODE_TEXT)};var x;d.guard=function(b,d){if(w(b))return true;if(a==CKEDITOR.SHRINK_ELEMENT&&b.type==CKEDITOR.NODE_TEXT||d&&b.equals(x)||
...
(!r||t)&&q.equals(l);if(i&&!r)for(;!q.getNext(d)&&!o;){x=q.getParent();if(x.isBlockBoundary(this.forceBrBreak&&!u&&{br:1})){r=1;t=0;o||x.equals(l);i.setEndAt(x,CKEDITOR.POSITION_BEFORE_END);break}q=x;t=1;o=q.equals(l);w=1}t&&i.setEndAt(q,CKEDITOR.POSITION_AFTER_END);q=this._getNextSourceNode(q,w,l);if((o=!q)||r&&i)break}if(!f){if(!i){this._.docEndMarker&&this._.docEndMarker.remove();return this._.nextNode=null}f=new CKEDITOR.dom.elementPath(i.startContainer,i.root);q=f.blockLimit;r={div:1,th:1,td:1};
...
/\s+/)||null;h.propertiesOnly=h.propertiesOnly||h.elements===true;var j=/\s*,\s*/,q=void 0;for(q in E){h[q]=i(h[q],j)||null;var k=h,n=D[q],x=i(h[D[q]],j),u=h[q],l=[],w=true,m=void 0;x?w=false:x={};for(m in u)if(m.charAt(0)=="!"){m=m.slice(1);l.push(m);x[m]=true;w=false}for(;m=l.pop();){u[m]=u["!"+m];delete u["!"+m]}k[n]=(w?false:x)||null}h.match=h.match||null;d.push(f);a.push(f)}for(var b=e.elements,e=e.generic,G,c=0,d=a.length;c<d;++c){g=F(a[c]);f=g.classes===true||g.styles===true||g.attributes===
...
allStyles:false,hadInvalidAttribute:false,hadInvalidClass:false,hadInvalidStyle:false};var r,u;if(!a&&!h)a=null;else{o(b);if(i){r=0;for(u=i.length;r<u;++r)if(e(i[r],b,n)===false){a=null;break a}}if(j){r=0;for(u=j.length;r<u;++r)e(j[r],b,n)}if(a){r=0;for(u=a.length;r<u;++r)f(a[r],b,n,k)}if(h){r=0;for(u=h.length;r<u;++r)f(h[r],b,n,k)}a=n}}if(!a){c.push(b);return A}if(!a.valid){c.push(b);return A}u=a.validAttributes;var m=a.validStyles;h=a.validClasses;var i=b.attributes,w=b.styles,j=b.classes,k=b.classBackup,
G=b.styleBackup,v,E,t=[];n=[];var s=/^data-cke-/;r=false;delete i.style;delete i["class"];delete b.classBackup;delete b.styleBackup;if(!a.allAttributes)for(v in i)if(!u[v])if(s.test(v)){if(v!=(E=v.replace(/^data-cke-saved-/,""))&&!u[E]){delete i[v];r=true}}else{delete i[v];r=true}if(!a.allStyles||a.hadInvalidStyle){for(v in w)a.allStyles||m[v]?t.push(v+":"+w[v]):r=true;if(t.length)i.style=t.sort().join("; ")}else if(G)i.style=G;if(!a.allClasses||a.hadInvalidClass){for(v=0;v<j.length;++v)(a.allClasses||
h[j[v]])&&n.push(j[v]);n.length&&(i["class"]=n.sort().join(" "));k&&n.length<k.split(/\s+/).length&&(r=true)}else k&&(i["class"]=k);r&&(g=A);if(!d.skipFinalValidation&&!q(b)){c.push(b);return A}}if(d.toHtml)b.name=b.name.replace(Q,"cke:$1");return g}function n(a){var b=[],c;for(c in a)c.indexOf("*")>-1&&b.push(c.replace(/\*/g,".*"));return b.length?RegExp("^(?:"+b.join("|")+")$"):null}function l(a){var b=a.attributes,c;delete b.style;delete b["class"];if(c=CKEDITOR.tools.writeCssText(a.styles,true))b.style=
...
else{a.parent&&c.push({check:"it",el:a.parent});a.replaceWithChildren()}}function x(a,b,c){var d,e;for(d=0;d<c.length;++d){e=c[d];if((!e.check||a.check(e.check,false))&&(!e.left||e.left(b))){e.right(b,O);break}}}function v(a,b){var c=b.getDefinition(),d=c.attributes,e=c.styles,g,f,h,i;if(a.name!=c.element)return false;for(g in d)if(g=="class"){c=d[g].split(/\s+/);for(h=a.classes.join("|");i=c.pop();)if(h.indexOf(i)==-1)return false}else if(a.attributes[g]!=d[g])return false;for(f in e)if(a.styles[f]!=
e[f])return false;return true}function s(a,b){var c,d;if(typeof a=="string")c=a;else if(a instanceof CKEDITOR.style)d=a;else{c=a[0];d=a[1]}return[{element:c,left:d,right:function(a,c){c.transform(a,b)}}]}function y(a){return function(b){return v(b,a)}}function z(a){return function(b,c){c[a](b)}}var B=CKEDITOR.dtd,A=1,F=CKEDITOR.tools.copy,C=CKEDITOR.tools.trim,J="cke-test",G=["","p","br","div"];CKEDITOR.FILTER_SKIP_TREE=2;CKEDITOR.filter=function(a){this.allowedContent=[];this.disallowedContent=[];
...
!c.children.length?w(c,d,a):q(c)||w(c,d,a);break;case "el-up":c.parent.type!=CKEDITOR.NODE_DOCUMENT_FRAGMENT&&!n[c.name]&&w(c,d,a);break;case "parent-down":c.parent.type!=CKEDITOR.NODE_DOCUMENT_FRAGMENT&&!n[c.name]&&w(c.parent,d,a)}}}return i},checkFeature:function(a){if(this.disabled||!a)return true;a.toFeature&&(a=a.toFeature(this.editor));return!a.requiredContent||this.check(a.requiredContent)},disable:function(){this.disabled=true},disallow:function(b){if(!g(this,b,true))return false;typeof b==
...
lengthToAttribute:function(a,b,c){c=c||b;if(!(c in a.attributes)){var d=a.styles[b],e=d&&d.match(/^(\d+)(?:\.\d*)?px$/);e?a.attributes[c]=e[1]:d==J&&(a.attributes[c]=J)}delete a.styles[b]},alignmentToStyle:function(a){if(!("float"in a.styles)){var b=a.attributes.align;if(b=="left"||b=="right")a.styles["float"]=b}delete a.attributes.align},alignmentToAttribute:function(a){if(!("align"in a.attributes)){var b=a.styles["float"];if(b=="left"||b=="right")a.attributes.align=b}delete a.styles["float"]},matchesStyle:v,
...
if(!r)if((c=a.previous)&&c.name=="br"||!c||g(c))return true}return false}var k={elements:{}},r=d=="html",n=CKEDITOR.tools.extend({},w),m;for(m in n)"#"in u[m]||delete n[m];for(m in n)k.elements[m]=h(r,a.config.fillEmptyBlocks);k.root=h(r,false);k.elements.br=function(a){return function(c){if(c.parent.type!=CKEDITOR.NODE_DOCUMENT_FRAGMENT){var d=c.attributes;if("data-cke-bogus"in d||"data-cke-eol"in d)delete d["data-cke-bogus"];else{for(d=c.next;d&&b(d);)d=d.next;var h=e(c);!d&&g(c.parent)?i(c.parent,
...
w||a.type==CKEDITOR.NODE_DOCUMENT_FRAGMENT)}function i(a,b){var c=a.children[a.children.length-1];a.children.push(b);b.parent=a;if(c){c.next=b;b.previous=c}}function d(a){a=a.attributes;a.contenteditable!="false"&&(a["data-cke-editable"]=a.contenteditable?"true":1);a.contenteditable="false"}function h(a){a=a.attributes;switch(a["data-cke-editable"]){case "true":a.contenteditable="true";break;case "1":delete a.contenteditable}}function j(a){return a.replace(z,function(a,b,c){return"<"+b+c.replace(B,
function(a,b){return A.test(b)&&c.indexOf("data-cke-saved-"+b)==-1?" data-cke-saved-"+a+" data-cke-"+CKEDITOR.rnd+"-"+a:a})+">"})}function k(a,b){return a.replace(b,function(a,b,c){a.indexOf("<textarea")===0&&(a=b+p(c).replace(/</g,"&lt;").replace(/>/g,"&gt;")+"</textarea>");return"<cke:encoded>"+encodeURIComponent(a)+"</cke:encoded>"})}function m(a){return a.replace(J,function(a,b){return decodeURIComponent(b)})}function o(a){return a.replace(/<\!--(?!{cke_protected})[\s\S]+?--\>/g,function(a){return"<\!--"+
...
c.addRules(v,{applyToAll:true});c.addRules(a(b,"data"),{applyToAll:true});d.addRules(s);d.addRules(y,{applyToAll:true});d.addRules(a(b,"html"),{applyToAll:true});b.on("toHtml",function(a){var a=a.data,c=a.dataValue,d,c=l(c,b),c=k(c,C),c=j(c),c=k(c,F),c=c.replace(G,"$1cke:$2"),c=c.replace(D,"<cke:$1$2></cke:$1>"),c=c.replace(/(<pre\b[^>]*>)(\r\n|\n)/g,"$1$2$2"),c=c.replace(/([^a-z0-9<\-])(on\w{3,})(?!>)/gi,"$1data-cke-"+CKEDITOR.rnd+"-$2");d=a.context||b.editable().getName();var e;if(CKEDITOR.env.ie&&
...
var n=function(){function a(b){return b.type==CKEDITOR.NODE_ELEMENT}function b(c,d){var e,f,g,h,i=[],k=d.range.startContainer;e=d.range.startPath();for(var k=j[k.getName()],n=0,m=c.getChildren(),w=m.count(),u=-1,x=-1,l=0,p=e.contains(j.$list);n<w;++n){e=m.getItem(n);if(a(e)){g=e.getName();if(p&&g in CKEDITOR.dtd.$list)i=i.concat(b(e,d));else{h=!!k[g];if(g=="br"&&e.data("cke-eol")&&(!n||n==w-1)){l=(f=n?i[n-1].node:m.getItem(n+1))&&(!a(f)||!f.is("br"));f=f&&a(f)&&j.$block[f.getName()]}u==-1&&!h&&(u=
...
s.root,l={type:l,dontFilter:G,editable:k,editor:o,range:s,blockLimit:E,mergeCandidates:[],zombies:[]},o=l.range,G=l.mergeCandidates,D,I,L,H;if(l.type=="text"&&o.shrink(CKEDITOR.SHRINK_ELEMENT,true,false)){D=CKEDITOR.dom.element.createFromHtml("<span>&nbsp;</span>",o.document);o.insertNode(D);o.setStartAfter(D)}I=new CKEDITOR.dom.elementPath(o.startContainer);l.endPath=L=new CKEDITOR.dom.elementPath(o.endContainer);if(!o.collapsed){var E=L.block||L.blockLimit,Q=o.getCommonAncestor();E&&(!E.equals(Q)&&
!E.contains(Q)&&o.checkEndOfBlock())&&l.zombies.push(E);o.deleteContents()}for(;(H=a(o.startContainer)&&o.startContainer.getChild(o.startOffset-1))&&a(H)&&H.isBlockBoundary()&&I.contains(H);)o.moveToPosition(H,CKEDITOR.POSITION_BEFORE_END);f(o,l.blockLimit,I,L);if(D){o.setEndBefore(D);o.collapse();D.remove()}D=o.startPath();if(E=D.contains(d,false,1)){o.splitElement(E);l.inlineStylesRoot=E;l.inlineStylesPeak=D.lastElement}D=o.createBookmark();(E=D.startNode.getPrevious(e))&&a(E)&&d(E)&&G.push(E);
...
M.remove()}}l.dataWrapper=H;D=p}if(D){M=l.range;var O=M.document,K,p=l.blockLimit;D=0;var R;H=[];var N,T,G=o=0,P,V;I=M.startContainer;var E=l.endPath.elements[0],W;L=E.getPosition(I);Q=!!E.getCommonAncestor(I)&&L!=CKEDITOR.POSITION_IDENTICAL&&!(L&CKEDITOR.POSITION_CONTAINS+CKEDITOR.POSITION_IS_CONTAINED);I=b(l.dataWrapper,l);for(g(I,M);D<I.length;D++){L=I[D];if(K=L.isLineBreak){K=M;P=p;var S=void 0,Y=void 0;if(L.hasBlockSibling)K=1;else{S=K.startContainer.getAscendant(j.$block,1);if(!S||!S.is({div:1,
...
R.remove();M.moveToPosition(T,CKEDITOR.POSITION_BEFORE_END)}if((K=M.startPath().block)&&!K.equals(N)){if(R=K.getBogus()){R.remove();H.push(K)}N=K}L.firstNotAllowed&&(o=1);if(o&&L.isElement){K=M.startContainer;for(P=null;K&&!j[K.getName()][L.name];){if(K.equals(p)){K=null;break}P=K;K=K.getParent()}if(K){if(P){V=M.splitElement(P);l.zombies.push(V);l.zombies.push(P)}}else{P=p.getName();W=!D;K=D==I.length-1;P=c(L.node,P);for(var S=[],Y=P.length,Z=0,ba=void 0,X=0,aa=-1;Z<Y;Z++){ba=P[Z];if(ba==" "){if(!X&&
(!W||Z)){S.push(new CKEDITOR.dom.text(" "));aa=S.length}X=1}else{S.push(ba);X=0}}K&&aa==S.length&&S.pop();W=S}}if(W){for(;K=W.pop();)M.insertNode(K);W=0}else M.insertNode(L.node);if(L.lastNotAllowed&&D<I.length-1){(V=Q?E:V)&&M.setEndAt(V,CKEDITOR.POSITION_AFTER_START);o=0}M.collapse()}}l.dontMoveCaret=G;l.bogusNeededBlocks=H}R=l.range;var U;V=l.bogusNeededBlocks;for(W=R.createBookmark();N=l.zombies.pop();)if(N.getParent()){T=R.clone();T.moveToElementEditStart(N);T.removeEmptyBlocksAtEnd()}if(V)for(;N=
...
if(!k){h=true;k=CKEDITOR.dtd.span}b.enlarge(CKEDITOR.ENLARGE_INLINE,1);b.trim();var n=b.createBookmark(),m=n.startNode,r=n.endNode,l=m,u;if(!j){var o=b.getCommonAncestor(),j=a(m,o),o=a(r,o);j&&(l=j.getNextSourceNode(true));o&&(r=o)}for(l.getPosition(r)==CKEDITOR.POSITION_FOLLOWING&&(l=0);l;){j=false;if(l.equals(r)){l=null;j=true}else{var t=l.type==CKEDITOR.NODE_ELEMENT?l.getName():null,o=t&&l.getAttribute("contentEditable")=="false",w=t&&l.getAttribute("data-nostyle");if(t&&l.data("cke-bookmark")){l=
l.getNextSourceNode(true);continue}if(o&&i&&CKEDITOR.dtd.$block[t])for(var x=l,v=e(x),s=void 0,z=v.length,y=0,x=z&&new CKEDITOR.dom.range(x.getDocument());y<z;++y){var s=v[y],A=CKEDITOR.filter.instances[s.data("cke-filter")];if(A?A.check(this):1){x.selectNodeContents(s);f.call(this,x)}}v=t?!k[t]||w?0:o&&!i?0:(l.getPosition(r)|F)==F&&(!g.childRule||g.childRule(l)):1;if(v)if((v=l.getParent())&&((v.getDtd()||CKEDITOR.dtd.span)[d]||h)&&(!g.parentRule||g.parentRule(v))){if(!u&&(!t||!CKEDITOR.dtd.$removeEmpty[t]||
(l.getPosition(r)|F)==F)){u=b.clone();u.setStartBefore(l)}t=l.type;if(t==CKEDITOR.NODE_TEXT||o||t==CKEDITOR.NODE_ELEMENT&&!l.getChildCount()){for(var t=l,aa;(j=!t.getNext(B))&&(aa=t.getParent(),k[aa.getName()])&&(aa.getPosition(m)|C)==C&&(!g.childRule||g.childRule(aa));)t=aa;u.setEndAfter(t)}}else j=true;else j=true;l=l.getNextSourceNode(w||o)}if(j&&u&&!u.collapsed){for(var j=q(this,c),o=j.hasAttributes(),w=u.getCommonAncestor(),t={},v={},s={},z={},U,$,ca;j&&w;){if(w.getName()==d){for(U in g.attributes)if(!z[U]&&
(ca=w.getAttribute($)))j.getAttribute(U)==ca?v[U]=1:z[U]=1;for($ in g.styles)if(!s[$]&&(ca=w.getStyle($)))j.getStyle($)==ca?t[$]=1:s[$]=1}w=w.getParent()}for(U in v)j.removeAttribute(U);for($ in t)j.removeStyle($);o&&!j.hasAttributes()&&(j=null);if(j){u.extractContents().appendTo(j);u.insertNode(j);p.call(this,j);j.mergeSiblings();CKEDITOR.env.ie||j.$.normalize()}else{j=new CKEDITOR.dom.element("span");u.extractContents().appendTo(j);u.insertNode(j);p.call(this,j);j.remove(true)}u=null}}b.moveToBookmark(n);
...
d=c.attributes,c=c.styles,e=t(this)[a.getName()],f=CKEDITOR.tools.isEmpty(d)&&CKEDITOR.tools.isEmpty(c),g;for(g in d)if(!((g=="class"||this._.definition.fullMatch)&&a.getAttribute(g)!=w(g,d[g]))&&!(b&&g.slice(0,5)=="data-")){f=a.hasAttribute(g);a.removeAttribute(g)}for(var h in c)if(!(this._.definition.fullMatch&&a.getStyle(h)!=w(h,c[h],true))){f=f||!!a.getStyle(h);a.removeStyle(h)}n(a,e,v[a.getName()]);f&&(this._.definition.alwaysRemoveElement?l(a,1):!CKEDITOR.dtd.$block[a.getName()]||this._.enterMode==
...
"aria-labelledby":this._.labelId};e.push('<div class="cke_dialog_ui_input_',c.type,'" role="presentation"');c.width&&e.push('style="width:'+c.width+'" ');e.push(">");if(c.size!=void 0)j.size=c.size;if(c.multiple!=void 0)j.multiple=c.multiple;i(a);for(var l=0,q;l<c.items.length&&(q=c.items[l]);l++)g.push('<option value="',CKEDITOR.tools.htmlEncode(q[1]!==void 0?q[1]:q[0]).replace(/"/g,"&quot;"),'" /> ',CKEDITOR.tools.htmlEncode(q[0]));if(typeof a.inputStyle!="undefined")a.style=a.inputStyle;f.select=
...
n=f.html;n.charAt(0)!="<"&&(n="<span>"+n+"</span>");var l=f.focus;if(l){var q=this.focus;this.focus=function(){(typeof l=="function"?l:q).call(this);this.fire("focus")};if(f.isFocusable)this.isFocusable=this.isFocusable;this.keyboardFocusable=true}CKEDITOR.ui.dialog.uiElement.call(this,e,f,i,"span",null,null,"");i=i.join("").match(a);n=n.match(b)||["","",""];if(c.test(n[1])){n[1]=n[1].slice(0,-1);n[2]="/"+n[2]}g.push([n[1]," ",i[1]||"",n[2]].join(""))}}}(),fieldset:function(a,b,c,e,f){var g=f.label;
...
c.config.removeDialogTabs){m=c.config.removeDialogTabs.split(";");for(p=0;p<m.length;p++){u=m[p].split(":");if(u.length==2){var v=u[0];t[v]||(t[v]=[]);t[v].push(u[1])}}c._.removeDialogTabs=t}if(c._.removeDialogTabs&&(t=c._.removeDialogTabs[d]))for(p=0;p<t.length;p++)l.removeContents(t[p]);if(l.onLoad)this.on("load",l.onLoad);if(l.onShow)this.on("show",l.onShow);if(l.onHide)this.on("hide",l.onHide);if(l.onOk)this.on("ok",function(a){c.fire("saveSnapshot");setTimeout(function(){c.fire("saveSnapshot")},
...
g[d])if(g=u(g[d],b,c,d,true))return g}if(e)return null}a.push(b);return b},t=function(a,b,c){for(var d=0,e;e=a[d];d++){if(e.id==b)return a.splice(d,1);if(c&&e[c])if(e=t(e[c],b,c))return e}return null},w=function(a,b){this.dialog=a;for(var c=b.contents,d=0,e;e=c[d];d++)c[d]=e&&new h(a,e);CKEDITOR.tools.extend(this,b)};w.prototype={getContents:function(a){return r(this.contents,a)},getButton:function(a){return r(this.buttons,a)},addContents:function(a,b){return u(this.contents,a,b)},addButton:function(a,
b){return u(this.buttons,a,b)},removeContents:function(a){t(this.contents,a)},removeButton:function(a){t(this.buttons,a)}};h.prototype={get:function(a){return r(this.elements,a,"children")},add:function(a,b){return u(this.elements,a,b,"children")},remove:function(a){t(this.elements,a,"children")}};var x,v={},s,y={},z=function(a){var b=a.data.$.ctrlKey||a.data.$.metaKey,c=a.data.$.altKey,d=a.data.$.shiftKey,e=String.fromCharCode(a.data.$.keyCode);if((b=y[(b?"CTRL+":"")+(c?"ALT+":"")+(d?"SHIFT+":"")+
...
CKEDITOR.TRISTATE_OFF)}c=(b.block||b.blockLimit||a.editable()).getDirection(1);if(c!=(a._.selDir||a.lang.dir)){a._.selDir=c;a.fire("contentDirChanged",c)}},exec:function(d){var e=d.getSelection(),h=d.config.enterMode,i=e.getRanges();if(i&&i.length){for(var j={},q=e.createBookmarks(),i=i.createIterator(),r,u=0;r=i.getNextRange(1);){var t=r.getEnclosedNode();if(!t||t&&!(t.type==CKEDITOR.NODE_ELEMENT&&t.getName()in g))t=f(r,b,h);t&&a(t,c,d,j);var w=new CKEDITOR.dom.walker(r),x=q[u].startNode,v=q[u++].endNode;
w.evaluator=function(a){return!!(a.type==CKEDITOR.NODE_ELEMENT&&a.getName()in b&&!(a.getName()==(h==CKEDITOR.ENTER_P?"p":"div")&&a.getParent().type==CKEDITOR.NODE_ELEMENT&&a.getParent().getName()=="blockquote")&&a.getPosition(x)&CKEDITOR.POSITION_FOLLOWING&&(a.getPosition(v)&CKEDITOR.POSITION_PRECEDING+CKEDITOR.POSITION_CONTAINS)==CKEDITOR.POSITION_PRECEDING)};for(;t=w.next();)a(t,c,d,j);r=r.createIterator();for(r.enlargeBr=h!=CKEDITOR.ENTER_BR;t=r.getNextParagraph(h==CKEDITOR.ENTER_P?"p":"div");)a(t,
...
CKEDITOR.TRISTATE_OFF)},context:"blockquote",allowedContent:"blockquote",requiredContent:"blockquote"};CKEDITOR.plugins.add("blockquote",{init:function(f){if(!f.blockless){f.addCommand("blockquote",a);f.ui.addButton&&f.ui.addButton("Blockquote",{label:f.lang.blockquote.toolbar,command:"blockquote",toolbar:"blocks,10"})}}})}(),"use strict",function(){function a(a){function b(){var c=a.editable();c.on(y,function(a){(!CKEDITOR.env.ie||!x)&&u(a)});CKEDITOR.env.ie&&c.on("paste",function(b){if(!v){f();
b.data.preventDefault();u(b);o("paste")||a.openDialog("paste")}});if(CKEDITOR.env.ie){c.on("contextmenu",g,null,null,0);c.on("beforepaste",function(a){a.data&&(!a.data.$.ctrlKey&&!a.data.$.shiftKey)&&g()},null,null,0)}c.on("beforecut",function(){!x&&n(a)});var d;c.attachListener(CKEDITOR.env.ie?c:a.document.getDocumentElement(),"mouseup",function(){d=setTimeout(function(){t()},0)});a.on("destroy",function(){clearTimeout(d)});c.on("keyup",t)}function c(b){return{type:b,canUndo:b=="cut",startDisabled:true,
...
b.getRanges()[0];e=a.document.createText("");e.insertBefore(c);d.setStartBefore(e);d.setEndAfter(c);b.selectRanges([d]);setTimeout(function(){if(c.getParent()){e.remove();b.selectElement(c)}},0)}}}function l(b,c){var d=a.document,e=a.editable(),f=function(a){a.cancel()},g;if(!d.getById("cke_pastebin")){var h=a.getSelection(),j=h.createBookmarks();CKEDITOR.env.ie&&h.root.fire("selectionchange");var k=new CKEDITOR.dom.element((CKEDITOR.env.webkit||e.is("body"))&&!CKEDITOR.env.ie?"body":"div",d);k.setAttributes({id:"cke_pastebin",
...
CKEDITOR.env.gecko&&(b=b.replace(/(\s)<br>$/,"$1"));a.data.dataValue=b},null,null,3);g.on("paste",function(a){var a=a.data,j=a.type,k=a.dataValue,m,o=g.config.clipboard_defaultContentType||"html";m=j=="html"||a.preSniffing=="html"?"html":f(k);m=="htmlifiedtext"?k=c(g.config,k):j=="text"&&m=="html"&&(k=b(g.config,k,d||(d=e(g))));a.startsWithEOL&&(k='<br data-cke-eol="1">'+k);a.endsWithEOL&&(k=k+'<br data-cke-eol="1">');j=="auto"&&(j=m=="html"||o=="html"?"html":"text");a.type=j;a.dataValue=k;delete a.preSniffing;
...
CKEDITOR.document.getById(this._.id),d=c.getFirst(),b=b.data;b!=a.lang.dir?c.addClass("cke_"+b):c.removeClass("cke_ltr").removeClass("cke_rtl");d.setAttribute("style",CKEDITOR.skin.getIconStyle(q,b=="rtl",this.icon,this.iconOffset))},this);h||(d=d+"off");var q=l=this.name||this.command;if(this.icon&&!/\./.test(this.icon)){q=this.icon;this.icon=null}d={id:i,name:l,iconName:q,label:this.label,cls:this.className||"",state:d,ariaDisabled:d=="disabled"?"true":"false",title:this.title,titleJs:g.gecko&&
!g.hc?"":(this.title||"").replace("'",""),hasArrow:this.hasArrow?"true":"false",keydownFn:m,focusFn:o,clickFn:j,style:CKEDITOR.skin.getIconStyle(q,a.lang.dir=="rtl",this.icon,this.iconOffset),arrowHtml:this.hasArrow?f.output():""};c.output(d,b);if(this.onRender)this.onRender();return k},setState:function(a){if(this._.state==a)return false;this._.state=a;var b=CKEDITOR.document.getById(this._.id);if(b){b.setState(a,"cke_button");a==CKEDITOR.TRISTATE_DISABLED?b.setAttribute("aria-disabled",true):b.removeAttribute("aria-disabled");
...
3));k.setStyle("width",a+10+"px");a=j.element.$.scrollHeight;CKEDITOR.env.ie&&(CKEDITOR.env.quirks&&a>0)&&(a=a+((k.$.offsetHeight||0)-(k.$.clientHeight||0)+3));k.setStyle("height",a+"px");h._.currentBlock.element.setStyle("display","none").removeStyle("display")}else k.removeStyle("height");n&&(l=l-k.$.offsetWidth);k.setStyle("left",l+"px");var b=h.element.getWindow(),a=k.$.getBoundingClientRect(),b=b.getViewPaneSize(),c=a.width||a.right-a.left,e=a.height||a.bottom-a.top,f=n?a.right:b.width-a.left,
...
1)));f.focus();return false;case 27:e.focus();return false;case 13:case 32:d(a);return false}return true});e.on("selectionChange",function(){e.editable();for(var a=[],d=m.list=[],f=[],h=m.filters,i=true,w=e.elementPath().elements,x,v=w.length;v--;){var s=w[v],y=0;x=s.data("cke-display-name")?s.data("cke-display-name"):s.data("cke-real-element-type")?s.data("cke-real-element-type"):s.getName();i=s.hasAttribute("contenteditable")?s.getAttribute("contenteditable")=="true":i;!i&&!s.hasAttribute("contenteditable")&&
...
n=g.checkStartOfBlock(),l=g.checkEndOfBlock(),q=a.elementPath(g.startContainer),r=q.block,u=d==CKEDITOR.ENTER_DIV?"div":"p",t;if(n&&l){if(r&&(r.is("li")||r.getParent().is("li"))){r.is("li")||(r=r.getParent());g=r.getParent();t=g.getParent();var o=!r.hasPrevious(),w=!r.hasNext(),u=a.getSelection(),x=u.createBookmarks(),n=r.getDirection(1),l=r.getAttribute("class"),v=r.getAttribute("style"),s=t.getDirection(1)!=n,a=a.enterMode!=CKEDITOR.ENTER_BR||s||v||l;if(t.is("li"))if(o||w)r[o?"insertBefore":"insertAfter"](t);
else r.breakParent(t);else{if(a){if(q.block.is("li")){t=p.createElement(d==CKEDITOR.ENTER_P?"p":"div");s&&t.setAttribute("dir",n);v&&t.setAttribute("style",v);l&&t.setAttribute("class",l);r.moveChildren(t)}else t=q.block;if(o||w)t[o?"insertBefore":"insertAfter"](g);else{r.breakParent(g);t.insertAfter(g)}}else{r.appendBogus(true);if(o||w)for(;p=r[o?"getFirst":"getLast"]();)p[o?"insertBefore":"insertAfter"](g);else for(r.breakParent(g);p=r.getLast();)p.insertAfter(g)}r.remove()}u.selectBookmarks(x);
...
(o=true);h.push(l);l=l.getNext()}if(h.length<1)return false;l=f.getParents(true);for(n=0;n<l.length;n++)if(l[n].getName&&i[l[n].getName()]){f=l[n];break}for(var l=e.isIndent?1:-1,n=h[0],h=h[h.length-1],o=CKEDITOR.plugins.list.listToArray(f,g),x=o[h.getCustomData("listarray_index")].indent,n=n.getCustomData("listarray_index");n<=h.getCustomData("listarray_index");n++){o[n].indent=o[n].indent+l;if(l>0){var v=o[n].parent;o[n].parent=new CKEDITOR.dom.element(v.getName(),v.getDocument())}}for(n=h.getCustomData("listarray_index")+
...
b){if(b&&b.data("cke-real-element-type")&&b.data("cke-real-element-type")=="anchor"){var c=a.restoreRealElement(b);if(c.data("cke-saved-name"))return c}},parseLinkAttributes:function(a,c){var e=c&&(c.data("cke-saved-href")||c.getAttribute("href"))||"",f=a.plugins.link.compiledProtectionFunction,w=a.config.emailProtection,x,v={};e.match(b)&&(w=="encode"?e=e.replace(m,function(a,b,c){return"mailto:"+String.fromCharCode.apply(String,b.split(","))+(c&&c.replace(/\\'/g,"'"))}):w&&e.replace(o,function(a,
b,c){if(b==f.name){v.type="email";for(var a=v.email={},b=/(^')|('$)/g,c=c.match(/[^,\s]+/g),d=c.length,e,g,h=0;h<d;h++){e=decodeURIComponent;g=c[h].replace(b,"").replace(/\\'/g,"'");g=e(g);e=f.params[h].toLowerCase();a[e]=g}a.address=[a.name,a.domain].join("@")}}));if(!v.type)if(w=e.match(h)){v.type="anchor";v.anchor={};v.anchor.name=v.anchor.id=w[1]}else if(w=e.match(g)){x=e.match(i);e=e.match(d);v.type="email";var s=v.email={};s.address=w[1];x&&(s.subject=decodeURIComponent(x[1]));e&&(s.body=decodeURIComponent(e[1]))}else if(e&&
(x=e.match(j))){v.type="url";v.url={};v.url.protocol=x[1];v.url.url=x[2]}if(c){if(e=c.getAttribute("target"))v.target={type:e.match(k)?e:"frame",name:e};else if(e=(e=c.data("cke-pa-onclick")||c.getAttribute("onclick"))&&e.match(p))for(v.target={type:"popup",name:e[1]};w=n.exec(e[2]);)(w[2]=="yes"||w[2]=="1")&&!(w[1]in{height:1,width:1,top:1,left:1})?v.target[w[1]]=true:isFinite(w[2])&&(v.target[w[1]]=w[2]);var e={},y;for(y in l)(w=c.getAttribute(y))&&(e[l[y]]=w);if(y=c.data("cke-saved-name")||e.advName)e.advName=
y;if(!CKEDITOR.tools.isEmpty(e))v.advanced=e}return v},getLinkAttributes:function(b,d){var e=b.config.emailProtection||"",g={};switch(d.type){case "url":var e=d.url&&d.url.protocol!=void 0?d.url.protocol:"http://",h=d.url&&CKEDITOR.tools.trim(d.url.url)||"";g["data-cke-saved-href"]=h.indexOf("/")===0?h:e+h;break;case "anchor":e=d.anchor&&d.anchor.id;g["data-cke-saved-href"]="#"+(d.anchor&&d.anchor.name||e||"");break;case "email":var i=d.email,h=i.address;switch(e){case "":case "encode":var j=encodeURIComponent(i.subject||
...
"toolbar","menubar","fullscreen","scrollbars","dependent"],h=n.length,j=function(a){d.target[a]&&n.push(a+"="+d.target[a])},i=0;i<h;i++)n[i]=n[i]+(d.target[n[i]]?"=yes":"=no");j("width");j("left");j("height");j("top");e.push(n.join(","),"'); return false;");g["data-cke-pa-onclick"]=e.join("")}else if(d.target.type!="notSet"&&d.target.name)g.target=d.target.name;if(d.advanced){for(var m in l)(e=d.advanced[l[m]])&&(g[m]=e);if(g.name)g["data-cke-saved-name"]=g.name}if(g["data-cke-saved-href"])g.href=
g["data-cke-saved-href"];m=CKEDITOR.tools.extend({target:1,onclick:1,"data-cke-pa-onclick":1,"data-cke-saved-name":1},l);for(var p in g)delete m[p];return{set:g,removed:CKEDITOR.tools.objectKeys(m)}}};CKEDITOR.unlinkCommand=function(){};CKEDITOR.unlinkCommand.prototype={exec:function(a){var b=new CKEDITOR.style({element:"a",type:CKEDITOR.STYLE_INLINE,alwaysRemoveElement:1});a.removeStyle(b)},refresh:function(a,b){var c=b.lastElement&&b.lastElement.getAscendant("a",true);c&&c.getName()=="a"&&c.getAttribute("href")&&
...
"li"){var j={parent:a,indent:e,element:i,contents:[]};if(f)j.grandparent=f;else{j.grandparent=a.getParent();if(j.grandparent&&j.grandparent.$.nodeName.toLowerCase()=="li")j.grandparent=j.grandparent.getParent()}b&&CKEDITOR.dom.element.setMarker(b,i,"listarray_index",c.length);c.push(j);for(var k=0,m=i.getChildCount(),p;k<m;k++){p=i.getChild(k);p.type==CKEDITOR.NODE_ELEMENT&&d[p.getName()]?CKEDITOR.plugins.list.listToArray(p,b,c,e+1,j.grandparent):j.contents.push(p)}}}return c},arrayToList:function(a,
...
1));m++}else if(C.indent==Math.max(o,0)+1){C=a[m-1].element.getDirection(1);m=CKEDITOR.plugins.list.arrayToList(a,null,m,e,C!=B?B:null);!z.getChildCount()&&(CKEDITOR.env.needsNbspFiller&&!(h.$.documentMode>7))&&z.append(h.createText(" "));z.append(m.listNode);m=m.nextIndex}else if(C.indent==-1&&!c&&g){if(d[g.getName()]){z=C.element.clone(false,true);B!=g.getDirection(1)&&z.setAttribute("dir",B)}else z=new CKEDITOR.dom.documentFragment(h);var p=g.getDirection(1)!=B,J=C.element,G=J.getAttribute("class"),
E=J.getAttribute("style"),D=z.type==CKEDITOR.NODE_DOCUMENT_FRAGMENT&&(e!=CKEDITOR.ENTER_BR||p||E||G),I,L=C.contents.length,H;for(g=0;g<L;g++){I=C.contents[g];if(j(I)&&L>1)D?H=I.clone(1,1):z.append(I.clone(1,1));else if(I.type==CKEDITOR.NODE_ELEMENT&&I.isBlockBoundary()){p&&!I.getDirection()&&I.setAttribute("dir",B);A=I;var Q=J.getAttribute("style");Q&&A.setAttribute("style",Q.replace(/([^;])$/,"$1;")+(A.getAttribute("style")||""));G&&I.addClass(G);A=null;if(H){z.append(H);H=null}z.append(I.clone(1,
1))}else if(D){if(!A){A=h.createElement(F);z.append(A);p&&A.setAttribute("dir",B)}E&&A.setAttribute("style",E);G&&A.setAttribute("class",G);if(H){A.append(H);H=null}A.append(I.clone(1,1))}else z.append(I.clone(1,1))}if(H){(A||z).append(H);H=null}if(z.type==CKEDITOR.NODE_DOCUMENT_FRAGMENT&&m!=a.length-1){if(CKEDITOR.env.needsBrFiller)(B=z.getLast())&&(B.type==CKEDITOR.NODE_ELEMENT&&B.is("br"))&&B.remove();B=z.getLast(k);(!B||!(B.type==CKEDITOR.NODE_ELEMENT&&B.is(CKEDITOR.dtd.$block)))&&z.append(h.createElement("br"))}B=
...
f.createBookmarks(true),h=[],j={},g=g.createIterator(),m=0;(i=g.getNextRange())&&++m;){var p=i.getBoundaryNodes(),s=p.startNode,y=p.endNode;s.type==CKEDITOR.NODE_ELEMENT&&s.getName()=="td"&&i.setStartAt(p.startNode,CKEDITOR.POSITION_AFTER_START);y.type==CKEDITOR.NODE_ELEMENT&&y.getName()=="td"&&i.setEndAt(p.endNode,CKEDITOR.POSITION_BEFORE_END);i=i.createIterator();for(i.forceBrBreak=this.state==CKEDITOR.TRISTATE_OFF;p=i.getNextParagraph();)if(!p.getCustomData("list_block")){CKEDITOR.dom.element.setMarker(j,
p,"list_block",1);for(var z=b.elementPath(p),s=z.elements,y=0,z=z.blockLimit,B,A=s.length-1;A>=0&&(B=s[A]);A--)if(d[B.getName()]&&z.contains(B)){z.removeCustomData("list_group_object_"+m);if(s=B.getCustomData("list_group_object"))s.contents.push(p);else{s={root:B,contents:[p]};h.push(s);CKEDITOR.dom.element.setMarker(j,B,"list_group_object",s)}y=1;break}if(!y){y=z;if(y.getCustomData("list_group_object_"+m))y.getCustomData("list_group_object_"+m).contents.push(p);else{s={root:y,contents:[p]};CKEDITOR.dom.element.setMarker(j,
y,"list_group_object_"+m,s);h.push(s)}}}}for(B=[];h.length>0;){s=h.shift();if(this.state==CKEDITOR.TRISTATE_OFF)if(d[s.root.getName()]){g=b;m=s;s=j;i=B;y=CKEDITOR.plugins.list.listToArray(m.root,s);z=[];for(p=0;p<m.contents.length;p++){A=m.contents[p];if((A=A.getAscendant("li",true))&&!A.getCustomData("list_item_processed")){z.push(A);CKEDITOR.dom.element.setMarker(s,A,"list_item_processed",true)}}for(var A=m.root.getDocument(),F=void 0,C=void 0,p=0;p<z.length;p++){var J=z[p].getCustomData("listarray_index"),
F=y[J].parent;if(!F.is(this.type)){C=A.createElement(this.type);F.copyAttributes(C,{start:1,type:1});C.removeStyle("list-style-type");y[J].parent=C}}s=CKEDITOR.plugins.list.arrayToList(y,s,null,g.config.enterMode);y=void 0;z=s.listNode.getChildCount();for(p=0;p<z&&(y=s.listNode.getChild(p));p++)y.getName()==this.type&&i.push(y);s.listNode.replace(m.root);g.fire("contentDomInvalidated")}else{y=b;p=s;i=B;z=p.contents;g=p.root.getDocument();m=[];if(z.length==1&&z[0].equals(p.root)){s=g.createElement("div");
z[0].moveChildren&&z[0].moveChildren(s);z[0].append(s);z[0]=s}p=p.contents[0].getParent();for(A=0;A<z.length;A++)p=p.getCommonAncestor(z[A].getParent());F=y.config.useComputedState;y=s=void 0;F=F===void 0||F;for(A=0;A<z.length;A++)for(C=z[A];J=C.getParent();){if(J.equals(p)){m.push(C);!y&&C.getDirection()&&(y=1);C=C.getDirection(F);s!==null&&(s=s&&s!=C?null:C);break}C=J}if(!(m.length<1)){z=m[m.length-1].getNext();A=g.createElement(this.type);i.push(A);for(F=i=void 0;m.length;){i=m.shift();F=g.createElement("li");
if(i.is("pre")||o.test(i.getName())||i.getAttribute("contenteditable")=="false")i.appendTo(F);else{i.copyAttributes(F);if(s&&i.getDirection()){F.removeStyle("direction");F.removeAttribute("dir")}i.moveChildren(F);i.remove()}F.appendTo(A)}s&&y&&A.setAttribute("dir",s);z?A.insertBefore(z):A.appendTo(p)}}else this.state==CKEDITOR.TRISTATE_ON&&d[s.root.getName()]&&a.call(this,b,s,j)}for(A=0;A<B.length;A++)e(B[A]);CKEDITOR.dom.element.clearAllMarkers(j);f.selectBookmarks(c);b.focus()},refresh:function(a,
...
a.ui.addButton("BulletedList",{label:a.lang.list.bulletedlist,command:"bulletedlist",directional:true,toolbar:"list,20"})}a.on("key",function(c){var e=c.data.domEvent.getKey();if(a.mode=="wysiwyg"&&e in{8:1,46:1}){var f=a.getSelection().getRanges()[0],h=f&&f.startPath();if(f&&f.collapsed){var j=e==8,p=a.editable(),o=new CKEDITOR.dom.walker(f.clone());o.evaluator=function(a){return k(a)&&!m(a)};o.guard=function(a,b){return!(b&&a.type==CKEDITOR.NODE_ELEMENT&&a.is("table"))};e=f.clone();if(j){var v,
s;if((v=h.contains(d))&&f.checkBoundaryOfElement(v,CKEDITOR.START)&&(v=v.getParent())&&v.is("li")&&(v=i(v))){s=v;v=v.getPrevious(k);e.moveToPosition(v&&m(v)?v:s,CKEDITOR.POSITION_BEFORE_START)}else{o.range.setStartAt(p,CKEDITOR.POSITION_AFTER_START);o.range.setEnd(f.startContainer,f.startOffset);if((v=o.previous())&&v.type==CKEDITOR.NODE_ELEMENT&&(v.getName()in d||v.is("li"))){if(!v.is("li")){o.range.selectNodeContents(v);o.reset();o.evaluator=b;v=o.previous()}s=v;e.moveToElementEditEnd(s)}}if(s){g(a,
e,f);c.cancel()}else if((e=h.contains(d))&&f.checkBoundaryOfElement(e,CKEDITOR.START)){s=e.getFirst(k);if(f.checkBoundaryOfElement(s,CKEDITOR.START)){v=e.getPrevious(k);if(i(s)){if(v){f.moveToElementEditEnd(v);f.select()}}else a.execCommand("outdent");c.cancel()}}}else if(s=h.contains("li")){o.range.setEndAt(p,CKEDITOR.POSITION_BEFORE_END);p=(h=s.getLast(k))&&b(h)?h:s;s=0;if((v=o.next())&&v.type==CKEDITOR.NODE_ELEMENT&&v.getName()in d&&v.equals(h)){s=1;v=o.next()}else f.checkBoundaryOfElement(p,CKEDITOR.END)&&
(s=1);if(s&&v){f=f.clone();f.moveToElementEditStart(v);g(a,e,f);c.cancel()}}else{o.range.setEndAt(p,CKEDITOR.POSITION_BEFORE_END);if((v=o.next())&&v.type==CKEDITOR.NODE_ELEMENT&&v.is(d)){v=v.getFirst(k);if(h.block&&f.checkStartOfBlock()&&f.checkEndOfBlock()){h.block.remove();f.moveToElementEditStart(v);f.select()}else if(i(v)){f.moveToElementEditStart(v);f.select()}else{f=f.clone();f.moveToElementEditStart(v);g(a,e,f)}c.cancel()}}setTimeout(function(){a.selectionChange(1)})}}})}}})}(),function(){CKEDITOR.plugins.liststyle=
...
CKEDITOR.plugins.add("liststyle",CKEDITOR.plugins.liststyle)}(),"use strict",function(){function a(a,b,c){return k(b)&&k(c)&&c.equals(b.getNext(function(a){return!(Y(a)||Z(a)||m(a))}))}function f(a){this.upper=a[0];this.lower=a[1];this.set.apply(this,a.slice(2))}function c(a){var b=a.element;if(b&&k(b))if((b=b.getAscendant(a.triggers,true))&&a.editable.contains(b)){var c=g(b);if(c.getAttribute("contenteditable")=="true")return b;if(c.is(a.triggers))return c}return null}function e(a,b,c){t(a,b);t(a,
c);a=b.size.bottom;c=c.size.top;return a&&c?0|(a+c)/2:a||c}function b(a,b,c){return b=b[c?"getPrevious":"getNext"](function(b){return b&&b.type==CKEDITOR.NODE_TEXT&&!Y(b)||k(b)&&!m(b)&&!j(a,b)})}function g(a,b){if(a.data("cke-editable"))return null;for(b||(a=a.getParent());a;){if(a.data("cke-editable"))break;if(a.hasAttribute("contenteditable"))return a;a=a.getParent()}return null}function i(a){var b=a.doc,c=z('<span contenteditable="false" style="'+P+"position:absolute;border-top:1px dashed "+a.boxColor+
...
a.boxColor+";",looks:["border-width:8px 8px 8px 0;top:-8px","border-width:8px 8px 0 0;top:-8px","border-width:0 8px 8px 0;top:0px"]})],detach:function(){this.wrap.getParent()&&this.wrap.remove();return this},mouseNear:function(){t(a,this);var b=a.holdDistance,c=this.size;return c&&a.mouse.y>c.top-b&&a.mouse.y<c.bottom+b&&a.mouse.x>c.left-b&&a.mouse.x<c.right+b?true:false},place:function(){var b=a.view,c=a.editable,d=a.trigger,e=d.upper,f=d.lower,g=e||f,h=g.getParent(),i={};this.trigger=d;e&&t(a,e,
true);f&&t(a,f,true);t(a,h,true);a.inInlineMode&&w(a,true);if(h.equals(c)){i.left=b.scroll.x;i.right=-b.scroll.x;i.width=""}else{i.left=g.size.left-g.size.margin.left+b.scroll.x-(a.inInlineMode?b.editable.left+b.editable.border.left:0);i.width=g.size.outerWidth+g.size.margin.left+g.size.margin.right+b.scroll.x;i.right=""}if(e&&f)i.top=e.size.margin.bottom===f.size.margin.top?0|e.size.bottom+e.size.margin.bottom/2:e.size.margin.bottom<f.size.margin.top?e.size.bottom+e.size.margin.bottom:e.size.bottom+
...
a.lastCmdDirection=!!e});!B.ie&&a.enterMode!=CKEDITOR.ENTER_BR&&a.hotNode.scrollIntoView();a.line.detach()}return function(d){var d=d.getSelection().getStartElement(),h,d=d.getAscendant(N,1);if(!n(a,d)&&d&&!d.equals(a.editable)&&!d.contains(a.editable)){if((h=g(d))&&h.getAttribute("contenteditable")=="false")d=h;a.element=d;h=b(a,d,!e);var i;if(k(h)&&h.is(a.triggers)&&h.is(R)&&(!b(a,h,!e)||(i=b(a,h,!e))&&k(i)&&i.is(a.triggers)))f(h);else{i=c(a,d);if(k(i))if(b(a,i,!e))(d=b(a,i,!e))&&(k(d)&&d.is(a.triggers))&&
f(i);else f(i)}}}}()}}function j(a,b){if(!b||!(b.type==CKEDITOR.NODE_ELEMENT&&b.$))return false;var c=a.line;return c.wrap.equals(b)||c.wrap.contains(b)}function k(a){return a&&a.type==CKEDITOR.NODE_ELEMENT&&a.$}function m(a){if(!k(a))return false;var b;if(!(b=o(a)))if(k(a)){b={left:1,right:1,center:1};b=!(!b[a.getComputedStyle("float")]&&!b[a.getAttribute("align")])}else b=false;return b}function o(a){return!!{absolute:1,fixed:1}[a.getComputedStyle("position")]}function p(a,b){return k(b)?b.is(a.triggers):
null}function n(a,b){if(!b)return false;for(var c=b.getParents(1),d=c.length;d--;)for(var e=a.tabuList.length;e--;)if(c[d].hasAttribute(a.tabuList[e]))return true;return false}function l(a,b,c){b=b[c?"getLast":"getFirst"](function(b){return a.isRelevant(b)&&!b.is(K)});if(!b)return false;t(a,b);return c?b.size.top>a.mouse.y:b.size.bottom<a.mouse.y}function q(a){var b=a.editable,c=a.mouse,d=a.view,e=a.triggerOffset;w(a);var g=c.y>(a.inInlineMode?d.editable.top+d.editable.height/2:Math.min(d.editable.height,
...
null,G,D,a])}return null}function r(a){var d=a.mouse,e=a.view,g=a.triggerOffset,h=c(a);if(!h)return null;t(a,h);var g=Math.min(g,0|h.size.outerHeight/2),i=[],j,n;if(d.y>h.size.top-1&&d.y<h.size.top+g)n=false;else if(d.y>h.size.bottom-g&&d.y<h.size.bottom+1)n=true;else return null;if(m(h)||l(a,h,n)||h.getParent().is(M))return null;var r=b(a,h,!n);if(r){if(r&&r.type==CKEDITOR.NODE_TEXT)return null;if(k(r)){if(m(r)||!p(a,r)||r.getParent().is(M))return null;i=[r,h][n?"reverse":"concat"]().concat([E,D])}}else{if(h.equals(a.editable[n?
"getLast":"getFirst"](a.isRelevant))){w(a);n&&d.y>h.size.bottom-g&&d.y<e.pane.height&&h.size.bottom>e.pane.height-g&&h.size.bottom<e.pane.height?j=H:d.y>0&&d.y<h.size.top+g&&(j=L)}else j=Q;i=[null,h][n?"reverse":"concat"]().concat([n?G:J,D,j,h.equals(a.editable[n?"getLast":"getFirst"](a.isRelevant))?n?H:L:Q])}return 0 in i?new f(i):null}function u(a,b,c,d){for(var e=function(){var c=B.ie?b.$.currentStyle:a.win.$.getComputedStyle(b.$,"");return B.ie?function(a){return c[CKEDITOR.tools.cssStyleToDomStyle(a)]}:
...
j.top+j.outerHeight;j.right=j.left+j.outerWidth;if(a.inInlineMode)j.scroll={top:b.$.scrollTop,left:b.$.scrollLeft};return s({border:g,padding:i,margin:h,ignoreScroll:c},j,true)}function t(a,b,c){if(!k(b))return b.size=null;if(b.size){if(b.size.ignoreScroll==c&&b.size.date>new Date-T)return null}else b.size={};return s(b.size,u(a,b,c),{date:+new Date},true)}function w(a,b){a.view.editable=u(a,a.editable,b,true)}function x(a,b){if(!a.view)a.view={};var c=a.view;if(b||!(c&&c.date>new Date-T)){var d=
a.win,c=d.getScrollPosition(),d=d.getViewPaneSize();s(a.view,{scroll:{x:c.x,y:c.y,width:a.doc.$.documentElement.scrollWidth-d.width,height:a.doc.$.documentElement.scrollHeight-d.height},pane:{width:d.width,height:d.height,bottom:d.height+c.y},date:+new Date},true)}}function v(a,b,c,d){for(var e=d,g=d,h=0,i=false,j=false,k=a.view.pane.height,n=a.mouse;n.y+h<k&&n.y-h>0;){i||(i=b(e,d));j||(j=b(g,d));!i&&n.y-h>0&&(e=c(a,{x:n.x,y:n.y-h}));!j&&n.y+h<k&&(g=c(a,{x:n.x,y:n.y+h}));if(i&&j)break;h=h+2}return new f([e,
g,null,null])}CKEDITOR.plugins.add("magicline",{init:function(a){var e=a.config,g=e.magicline_triggerOffset||30,l={editor:a,enterMode:e.enterMode,triggerOffset:g,holdDistance:0|g*(e.magicline_holdDistance||0.5),boxColor:e.magicline_color||"#ff0000",rtl:e.contentsLangDirection=="rtl",tabuList:["data-cke-hidden-sel"].concat(e.magicline_tabuList||[]),triggers:e.magicline_everywhere?N:{table:1,hr:1,div:1,ul:1,ol:1,dl:1,form:1,blockquote:1}},p,t,v;l.isRelevant=function(a){return k(a)&&!j(l,a)&&!m(a)};
a.on("contentDom",function(){var g=a.editable(),k=a.document,m=a.window;s(l,{editable:g,inInlineMode:g.isInline(),doc:k,win:m,hotNode:null},true);l.boundary=l.inInlineMode?l.editable:l.doc.getDocumentElement();if(!g.is(F.$inline)){l.inInlineMode&&!o(g)&&g.setStyles({position:"relative",top:null,left:null});i.call(this,l);x(l);g.attachListener(a,"beforeUndoImage",function(){l.line.detach()});g.attachListener(a,"beforeGetData",function(){if(l.line.wrap.getParent()){l.line.detach();a.once("getData",
function(){l.line.attach()},null,null,1E3)}},null,null,0);g.attachListener(l.inInlineMode?k:k.getWindow().getFrame(),"mouseout",function(b){if(a.mode=="wysiwyg")if(l.inInlineMode){var c=b.data.$.clientX,b=b.data.$.clientY;x(l);w(l,true);var d=l.view.editable,e=l.view.scroll;if(!(c>d.left-e.x&&c<d.right-e.x)||!(b>d.top-e.y&&b<d.bottom-e.y)){clearTimeout(v);v=null;l.line.detach()}}else{clearTimeout(v);v=null;l.line.detach()}});g.attachListener(g,"keyup",function(){l.hiddenMode=0});g.attachListener(g,
"keydown",function(b){if(a.mode=="wysiwyg"){b=b.data.getKeystroke();a.getSelection().getStartElement();switch(b){case 2228240:case 16:l.hiddenMode=1;l.line.detach()}}});g.attachListener(l.inInlineMode?g:k,"mousemove",function(b){t=true;if(!(a.mode!="wysiwyg"||a.readOnly||v)){var c={x:b.data.$.clientX,y:b.data.$.clientY};v=setTimeout(function(){l.mouse=c;v=l.trigger=null;x(l);if(t&&!l.hiddenMode&&a.focusManager.hasFocus&&!l.line.mouseNear()&&(l.element=S(l,true))){if((l.trigger=q(l)||r(l)||ba(l))&&
!n(l,l.trigger.upper||l.trigger.lower))l.line.attach().place();else{l.trigger=null;l.line.detach()}t=false}},30)}});g.attachListener(m,"scroll",function(){if(a.mode=="wysiwyg"){l.line.detach();if(B.webkit){l.hiddenMode=1;clearTimeout(p);p=setTimeout(function(){if(!l.mouseDown)l.hiddenMode=0},50)}}});g.attachListener(A?k:m,"mousedown",function(){if(a.mode=="wysiwyg"){l.line.detach();l.hiddenMode=1;l.mouseDown=1}});g.attachListener(A?k:m,"mouseup",function(){l.hiddenMode=0;l.mouseDown=0});a.addCommand("accessPreviousSpace",
h(l));a.addCommand("accessNextSpace",h(l,true));a.setKeystroke([[e.magicline_keystrokePrevious,"accessPreviousSpace"],[e.magicline_keystrokeNext,"accessNextSpace"]]);a.on("loadSnapshot",function(){var b,c,d,e;for(e in{p:1,br:1,div:1}){b=a.document.getElementsByTag(e);for(d=b.count();d--;)if((c=b.getItem(d)).data("cke-magicline-hot")){l.hotNode=c;l.lastCmdDirection=c.data("cke-magicline-dir")==="true"?true:false;return}}});this.backdoor={accessFocusSpace:d,boxTrigger:f,isLine:j,getAscendantTrigger:c,
getNonEmptyNeighbour:b,getSize:u,that:l,triggerEdge:r,triggerEditable:q,triggerExpand:ba}}},this)}});var s=CKEDITOR.tools.extend,y=CKEDITOR.dom.element,z=y.createFromHtml,B=CKEDITOR.env,A=CKEDITOR.env.ie&&CKEDITOR.env.version<9,F=CKEDITOR.dtd,C={},J=128,G=64,E=32,D=16,I=8,L=4,H=2,Q=1,O=" ",M=F.$listItem,K=F.$tableContent,R=s({},F.$nonEditable,F.$empty),N=F.$block,T=100,P="width:0px;height:0px;padding:0px;margin:0px;display:block;z-index:9999;color:#fff;position:absolute;font-size: 0px;line-height:0px;",
...
false);e.setAttribute("contentEditable",true)}}if(a.editable().hasFocus){a.toolbox.focus();a.focus()}}}CKEDITOR.plugins.add("maximize",{init:function(a){function f(){var b=j.getViewPaneSize();a.resize(b.width,b.height,null,true)}if(a.elementMode!=CKEDITOR.ELEMENT_MODE_INLINE){var d=a.lang,h=CKEDITOR.document,j=h.getWindow(),k,m,o,p=CKEDITOR.TRISTATE_OFF;a.addCommand("maximize",{modes:{wysiwyg:!CKEDITOR.env.iOS,source:!CKEDITOR.env.iOS},readOnly:1,editorFocus:false,exec:function(){var n=a.container.getFirst(function(a){return a.type==
CKEDITOR.NODE_ELEMENT&&a.hasClass("cke_inner")}),l=a.ui.space("contents");if(a.mode=="wysiwyg"){var q=a.getSelection();k=q&&q.getRanges();m=j.getScrollPosition()}else{var r=a.editable().$;k=!CKEDITOR.env.ie&&[r.selectionStart,r.selectionEnd];m=[r.scrollLeft,r.scrollTop]}if(this.state==CKEDITOR.TRISTATE_OFF){j.on("resize",f);o=j.getScrollPosition();for(q=a.container;q=q.getParent();){q.setCustomData("maximize_saved_styles",c(q));q.setStyle("z-index",a.config.baseFloatZIndex-5)}l.setCustomData("maximize_saved_styles",
c(l,true));n.setCustomData("maximize_saved_styles",c(n,true));l={overflow:CKEDITOR.env.webkit?"":"hidden",width:0,height:0};h.getDocumentElement().setStyles(l);!CKEDITOR.env.gecko&&h.getDocumentElement().setStyle("position","fixed");(!CKEDITOR.env.gecko||!CKEDITOR.env.quirks)&&h.getBody().setStyles(l);CKEDITOR.env.ie?setTimeout(function(){j.$.scrollTo(0,0)},0):j.$.scrollTo(0,0);n.setStyle("position",CKEDITOR.env.gecko&&CKEDITOR.env.quirks?"fixed":"absolute");n.$.offsetLeft;n.setStyles({"z-index":a.config.baseFloatZIndex-
5,left:"0px",top:"0px"});n.addClass("cke_maximized");f();l=n.getDocumentPosition();n.setStyles({left:-1*l.x+"px",top:-1*l.y+"px"});CKEDITOR.env.gecko&&b(a)}else if(this.state==CKEDITOR.TRISTATE_ON){j.removeListener("resize",f);l=[l,n];for(q=0;q<l.length;q++){e(l[q],l[q].getCustomData("maximize_saved_styles"));l[q].removeCustomData("maximize_saved_styles")}for(q=a.container;q=q.getParent();){e(q,q.getCustomData("maximize_saved_styles"));q.removeCustomData("maximize_saved_styles")}CKEDITOR.env.ie?setTimeout(function(){j.$.scrollTo(o.x,
o.y)},0):j.$.scrollTo(o.x,o.y);n.removeClass("cke_maximized");if(CKEDITOR.env.webkit){n.setStyle("display","inline");setTimeout(function(){n.setStyle("display","block")},0)}a.fire("resize")}this.toggleState();if(q=this.uiItems[0]){l=this.state==CKEDITOR.TRISTATE_OFF?d.maximize.maximize:d.maximize.minimize;q=CKEDITOR.document.getById(q._.id);q.getChild(1).setHtml(l);q.setAttribute("title",l);q.setAttribute("href",'javascript:void("'+l+'");')}if(a.mode=="wysiwyg")if(k){CKEDITOR.env.gecko&&b(a);a.getSelection().selectRanges(k);
(r=a.getSelection().getStartElement())&&r.scrollIntoView(true)}else j.$.scrollTo(m.x,m.y);else{if(k){r.selectionStart=k[0];r.selectionEnd=k[1]}r.scrollLeft=m[0];r.scrollTop=m[1]}k=m=null;p=this.state;a.fire("maximize",this.state)},canUndo:false});a.ui.addButton&&a.ui.addButton("Maximize",{label:d.maximize.maximize,command:"maximize",toolbar:"tools,10"});a.on("mode",function(){var b=a.getCommand("maximize");b.setState(b.state==CKEDITOR.TRISTATE_DISABLED?CKEDITOR.TRISTATE_DISABLED:p)},null,null,100)}}})}(),
...
"/")f.push('<span class="cke_toolbar_break"></span>');else{t=u.items||u;for(var w=0;w<t.length;w++){var x=t[w],v;if(x)if(x.type==CKEDITOR.UI_SEPARATOR)m=g&&x;else{v=x.canGroup!==false;if(!q){l=CKEDITOR.tools.getNextId();q={id:l,items:[]};r=u.name&&(b.lang.toolbar.toolbarGroups[u.name]||u.name);f.push('<span id="',l,'" class="cke_toolbar"',r?' aria-labelledby="'+l+'_label"':"",' role="toolbar">');r&&f.push('<span id="',l,'_label" class="cke_voice_label">',r,"</span>");f.push('<span class="cke_toolbar_start"></span>');
var s=o.push(q)-1;if(s>0){q.previous=o[s-1];q.previous.next=q}}if(v){if(!g){f.push('<span class="cke_toolgroup" role="presentation">');g=1}}else if(g){f.push("</span>");g=0}l=function(a){a=a.render(b,f);s=q.items.push(a)-1;if(s>0){a.previous=q.items[s-1];a.previous.next=a}a.toolbar=q;a.onkey=i;a.onfocus=function(){b.toolbox.focusCommandExecuted||b.focus()}};if(m){l(m);m=0}l(x)}}if(g){f.push("</span>");m=g=0}q&&f.push('<span class="cke_toolbar_end"></span></span>')}}}b.config.toolbarCanCollapse&&f.push("</span>");
