...
b=CKEDITOR.dom.element.createFromHtml(c.output({id:a.id,name:b,langDir:a.lang.dir,langCode:a.langCode,voiceLabel:"",topHtml:j?'<span id="'+a.ui.spaceId("top")+'" class="cke_top cke_reset_all" role="presentation" style="height:auto">'+j+"</span>":"",contentId:a.ui.spaceId("contents"),bottomHtml:i?'<span id="'+a.ui.spaceId("bottom")+'" class="cke_bottom cke_reset_all" role="presentation">'+i+"</span>":"",outerEl:CKEDITOR.env.ie?"span":"div"}));if(m==CKEDITOR.ELEMENT_MODE_REPLACE){e.hide();
...
this.toggleState()},canUndo:!0}}};function LineChanngelToOffSet(d,b){var f=b.line,j=f+b.ch;for(i=0;i<f;i++)j+=d.getLine(i).length;return j}function OffSetToLineChannel(d,b){var f=0,j=0,e=0;for(i=0;i<d.lineCount();i++){len=d.getLine(i).length;if(b<e+len){f=i;j=b-e;break}len++;e+=len}return{line:f,ch:j}};CKEDITOR.plugins.add("syntaxhighlight",{requires:"dialog",init:function(a){a.addCommand("syntaxhighlightDialog",new CKEDITOR.dialogCommand("syntaxhighlightDialog",{allowedContent:"pre[title](*)",requiredContent:"pre[title](*)"}));a.ui.addButton&&a.ui.addButton("Syntaxhighlight",{label:a.lang.syntaxhighlight.title,command:"syntaxhighlightDialog",toolbar:"insert,98"});a.contextMenu&&(a.addMenuGroup("syntaxhighlightGroup"),a.addMenuItem("syntaxhighlightItem",{label:a.lang.syntaxhighlight.contextTitle,
