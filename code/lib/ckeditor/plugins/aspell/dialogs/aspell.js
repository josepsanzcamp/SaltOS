...
var controlsWindow_end_spell; // by sanz
...
CKEDITOR.dialog.add('aspell', function( editor )
{
	...
	function spellTime(dialog, errorMsg)
	{
		...
		return function()
		{
			if (typeof window.spellChecker == 'function')
			{
				...
				oSpeller.spellCheckScript = editor.plugins.aspell.path+'spellerpages/server-scripts/spellchecker.php?lang='+CKEDITOR.config.language;
	...
	return {
		...
		minWidth: minW,
		minHeight: minH,
		buttons: [ CKEDITOR.dialog.okButton, // by sanz
				   CKEDITOR.dialog.cancelButton ],
		onOk: function() {
			controlsWindow_end_spell(); // by sanz
		},
		onShow: function()
		{
			...
			{
				controlsWindow_end_spell=controlsWindow.end_spell; // by sanz
		...
		onHide: function()
		{
			...
			FCKLang = null;
			controlsWindow_end_spell = null; // by sanz
		},
		contents: [
			{
				id: 'general',
