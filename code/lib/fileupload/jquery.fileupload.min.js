...
(!a.multipart&&c.support.xhrFileUpload||c.support.xhrFormDataFileUpload)},_getFormData:function(a){if("function"===c.type(a.formData))return a.formData(a.form);if(c.isArray(a.formData))return a.formData;if("object"===c.type(a.formData)){var b=[];c.each(a.formData,function(a,c){b.push({name:a,value:c})});return b}return[]},_getTotal:function(a){var b=0;c.each(a,function(a,c){b+=c.size||1});return b},_initProgressObject:function(a){var b={loaded:0,total:0,bitrate:0};a._progress?c.extend(a._progress,
b):a._progress=b},_initResponseObject:function(a){var b;if(a._response)for(b in a._response)Object.prototype.hasOwnProperty.call(a._response,b)&&delete a._response[b];else a._response={}},_onProgress:function(a,b){if(a.lengthComputable){var d=Date.now?Date.now():(new Date).getTime();if(!(b._time&&b.progressInterval&&d-b._time<b.progressInterval&&a.loaded!==a.total)){b._time=d;var e=Math.floor(a.loaded/a.total*(b.chunkSize||b._progress.total))+(b.uploadedBytes||0);this._progress.loaded+=e-b._progress.loaded;
...
d.lengthComputable=c.lengthComputable;d.loaded=c.loaded;d.total=c.total;b._onProgress(d,a)}),a.xhr=function(){return d})},_deinitProgressListener:function(a){a=a.xhr?a.xhr():c.ajaxSettings.xhr();a.upload&&c(a.upload).off("progress")},_isInstanceOf:function(a,b){return Object.prototype.toString.call(b)==="[object "+a+"]"},_getUniqueFilename:function(a,b){a=String(a);if(b[a])return a=a.replace(/(?: \(([\d]+)\))?(\.[^.]+)?$/,function(a,b,c){return" ("+(b?Number(b)+1:1)+")"+(c||"")}),this._getUniqueFilename(a,
b);b[a]=!0;return a},_initXHRData:function(a){var b=this,d=a.files[0],e=a.multipart||!c.support.xhrFileUpload,f="array"===c.type(a.paramName)?a.paramName[0]:a.paramName;a.headers=c.extend({},a.headers);a.contentRange&&(a.headers["Content-Range"]=a.contentRange);e&&!a.blob&&this._isInstanceOf("File",d)||(a.headers["Content-Disposition"]='attachment; filename="'+encodeURI(d.uploadName||d.name)+'"');if(!e)a.contentType=d.type||"application/octet-stream",a.data=a.blob||d;else if(c.support.xhrFormDataFileUpload){if(a.postMessage){var g=
this._getFormData(a);a.blob?g.push({name:f,value:a.blob}):c.each(a.files,function(b,d){g.push({name:"array"===c.type(a.paramName)&&a.paramName[b]||f,value:d})})}else b._isInstanceOf("FormData",a.formData)?g=a.formData:(g=new FormData,c.each(this._getFormData(a),function(a,b){g.append(b.name,b.value)})),a.blob?g.append(f,a.blob,d.uploadName||d.name):c.each(a.files,function(d,e){if(b._isInstanceOf("File",e)||b._isInstanceOf("Blob",e)){var m=e.uploadName||e.name;a.uniqueFilenames&&(m=b._getUniqueFilename(m,
...
loaded:f,total:f}),e);g.result=e.result=a;g.textStatus=e.textStatus=b;g.jqXHR=e.jqXHR=d;this._trigger("done",null,e)},_onFail:function(a,b,d,c){var e=c._response;c.recalculateProgress&&(this._progress.loaded-=c._progress.loaded,this._progress.total-=c._progress.total);e.jqXHR=c.jqXHR=a;e.textStatus=c.textStatus=b;e.errorThrown=c.errorThrown=d;this._trigger("fail",null,c)},_onAlways:function(a,b,d,c){this._trigger("always",null,c)},_onSend:function(a,b){b.submit||this._addConvenienceMethods(a,b);var d=
this,e,f,g=d._getAJAXSettings(b),l=function(){d._sending+=1;g._bitrateTimer=new d._BitrateTimer;return e=e||((f||!1===d._trigger("send",c.Event("send",{delegatedEvent:a}),g))&&d._getXHRPromise(!1,g.context,f)||d._chunkedUpload(g)||c.ajax(g)).done(function(a,b,c){d._onDone(a,b,c,g)}).fail(function(a,b,c){d._onFail(a,b,c,g)}).always(function(a,b,c){d._deinitProgressListener(g);d._onAlways(a,b,c,g);--d._sending;--d._active;if(g.limitConcurrentUploads&&g.limitConcurrentUploads>d._sending)for(a=d._slots.shift();a;){if("pending"===
d._getDeferredState(a)){a.resolve();break}a=d._slots.shift()}0===d._active&&d._trigger("stop")})};this._beforeSend(a,g);if(this.options.sequentialUploads||this.options.limitConcurrentUploads&&this.options.limitConcurrentUploads<=this._sending){if(1<this.options.limitConcurrentUploads){var k=c.Deferred();this._slots.push(k);b=k[d._promisePipe](l)}else b=this._sequence=this._sequence[d._promisePipe](l,l);b.abort=function(){f=[void 0,"abort","abort"];return e?e.abort():(k&&k.rejectWith(g.context,f),
l())};return this._enhancePromise(b)}return l()},_onAdd:function(a,b){var d=this,e=!0,f=c.extend({},this.options,b),g=b.files,l=g.length,k=f.limitMultiFileUploads,m=f.limitMultiFileUploadSize,n=f.limitMultiFileUploadSizeOverhead,h=0,r=this._getParamName(f),t=0;if(!l)return!1;m&&void 0===g[0].size&&(m=void 0);if((f.singleFileUploads||k||m)&&this._isXHRUpload(f))if(f.singleFileUploads||m||!k)if(!f.singleFileUploads&&m){var p=[];var q=[];for(f=0;f<l;f+=1)if(h+=g[f].size+n,f+1===l||h+g[f+1].size+n>m||
k&&f+1-t>=k)p.push(g.slice(t,f+1)),h=r.slice(t,f+1),h.length||(h=r),q.push(h),t=f+1,h=0}else q=r;else for(p=[],q=[],f=0;f<l;f+=k)p.push(g.slice(f,f+k)),h=r.slice(f,f+k),h.length||(h=r),q.push(h);else p=[g],q=[r];b.originalFiles=g;c.each(p||g,function(f,g){var h=c.extend({},b);h.files=p?g:[g];h.paramName=q[f];d._initResponseObject(h);d._initProgressObject(h);d._addConvenienceMethods(a,h);return e=d._trigger("add",c.Event("add",{delegatedEvent:a}),h)});return e},_replaceFileInput:function(a){var b=
a.fileInput,d=b.clone(!0),e=b.is(document.activeElement);a.fileInputClone=d;c("<form></form>").append(d)[0].reset();b.after(d).detach();e&&d.trigger("focus");c.cleanData(b.off("remove"));this.options.fileInput=this.options.fileInput.map(function(a,c){return c===b[0]?d[0]:c});b[0]===this.element[0]&&(this.element=d)},_handleFileTreeEntry:function(a,b){var d=this,e=c.Deferred(),f=[],g=function(b){b&&!b.entry&&(b.entry=a);e.resolve([b])},h=function(c){d._handleFileTreeEntries(c,b+a.name+"/").done(function(a){e.resolve(a)}).fail(g)},
k=function(){m.readEntries(function(a){a.length?(f=f.concat(a),k()):h(f)},g)};b=b||"";if(a.isFile)a._file?(a._file.relativePath=b,e.resolve(a._file)):a.file(function(a){a.relativePath=b;e.resolve(a)},g);else if(a.isDirectory){var m=a.createReader();k()}else e.resolve([]);return e.promise()},_handleFileTreeEntries:function(a,b){var d=this;return c.when.apply(c,c.map(a,function(a){return d._handleFileTreeEntry(a,b)}))[this._promisePipe](function(){return Array.prototype.concat.apply([],arguments)})},
