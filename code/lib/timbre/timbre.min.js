...
f.listeners=function(a){return this.events.listeners(a)};f.fix_iOS6_1_problem=function(a){this.impl.fix_iOS6_1_problem&&this.impl.fix_iOS6_1_problem(a)};return a}(),K=null,H=null;"undefined"!==typeof window&&(H=window.AudioContext||window.webkitAudioContext);var K="undefined"!==typeof H?function(a){var b=new H,f;m._audioContext=b;this.defaultSamplerate=this.maxSamplerate=b.sampleRate;this.env="webkit";var c=navigator.userAgent;c.match(/linux/i)?a.streammsec*=8:c.match(/win(dows)?\s*(nt 5\.1|xp)/i)&&
(a.streammsec*=4);this.play=function(){var c,j=a.getAdjustSamples(b.sampleRate),d=a.streamsize,l,e;if(a.samplerate===b.sampleRate)c=function(b){b=b.outputBuffer;a.process();b.getChannelData(0).set(a.strmL);b.getChannelData(1).set(a.strmR)};else if(a.samplerate*2===b.sampleRate)c=function(b){var f=a.strmL,c=a.strmR,j=b.outputBuffer,b=j.getChannelData(0),d=j.getChannelData(1),l=j.length,e;a.process();for(j=e=0;j<l;j=j+2,++e){b[j]=b[j+1]=f[e];d[j]=d[j+1]=c[e]}};else{l=d;e=a.samplerate/b.sampleRate;c=
function(b){for(var f=a.strmL,c=a.strmR,j=b.outputBuffer,b=j.getChannelData(0),p=j.getChannelData(1),g=j.length,j=0;j<g;++j){if(l>=d){a.process();l=l-d}b[j]=f[l|0];p[j]=c[l|0];l=l+e}}}f=b.createScriptProcessor(j,2,a.channels);f.onaudioprocess=c;f.connect(b.destination)};this.pause=function(){f.disconnect()};if(l){var j=0,d=b.createBufferSource();this.fix_iOS6_1_problem=function(a){j=j+(a?1:-1);if(j===1){d.noteOn(0);d.connect(b.destination)}else j===0&&d.disconnect()}}}:function(){this.maxSamplerate=
48E3;this.defaultSamplerate=44100;this.env="nop";this.play=function(){};this.pause=function(){}},b=(new r).bind(K),F=h;"node"===j?module.exports=global.timbre=F:"browser"===j&&(F.noConflict=function(){var a=window.timbre,b=window.T;return function(f){window.T===F&&(window.T=b);f&&window.timbre===F&&(window.timbre=a);return F}}(),window.timbre=window.T=F);(function(){if(!(b.impl.env!=="nop"||j!=="browser"||l)){var a=navigator,f;try{f=a.plugins&&a.mimeTypes&&a.mimeTypes.length?a.plugins["Shockwave Flash"].description.match(/([0-9]+)/)[0]:
(new ActiveXObject("ShockwaveFlash.ShockwaveFlash")).GetVariable("$version").match(/([0-9]+)/)[0]}catch(c){f=-1}if(!(f<10)){var d,e=function(){var a=document.getElementsByTagName("script");if(a&&a.length)for(var b,f=0,c=a.length;f<c;++f)if(b=/^(.*\/)timbre(?:\.dev)?\.js$/i.exec(a[f].src))return b[1]+"timbre.swf"}();window.timbrejs_flashfallback_init=function(){b.bind(function(a){var b=0;this.defaultSamplerate=this.maxSamplerate=44100;this.env="flash";this.play=function(){var f,c=Array(a.streamsize*
a.channels),j=a.streammsec,l=0,p=a.streamsize/a.samplerate*1E3,g=Date.now();f=function(){if(!(l>Date.now()-g)){var b=a.strmL,f=a.strmR,j=c.length,e=b.length;for(a.process();e--;){c[--j]=f[e]*32768|0;c[--j]=b[e]*32768|0}d.writeAudio(c.join(" "));l=l+p}};if(d.setup){d.setup(a.channels,a.samplerate);b=setInterval(f,j)}else console.warn("Cannot find "+e)};this.pause=function(){if(b!==0){d.cancel();clearInterval(b);b=0}}});delete window.timbrejs_flashfallback_init};f=e+"?"+ +new Date;var p=document.createElement("div");
p.id="TimbreFlashPlayerDiv";p.style.display="inline";p.width=p.height=1;if(a.plugins&&a.mimeTypes&&a.mimeTypes.length){a=document.createElement("object");a.id="TimbreFlashPlayer";a.classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000";a.width=a.height=1;a.setAttribute("data",f);a.setAttribute("type","application/x-shockwave-flash");f=document.createElement("param");f.setAttribute("name","allowScriptAccess");f.setAttribute("value","always");a.appendChild(f);p.appendChild(a)}else p.innerHTML='<object id="TimbreFlashPlayer" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="1" height="1"><param name="movie" value="'+
f+'" /><param name="bgcolor" value="#FFFFFF" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always" /></object>';window.addEventListener("load",function(){document.body.appendChild(p);d=document.TimbreFlashPlayer})}}})()})();
