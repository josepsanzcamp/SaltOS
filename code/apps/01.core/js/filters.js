...
if (typeof __filters__ == "undefined" && typeof parent.__filters__ == "undefined") {
    ...
    function load_filter()
    {
        ...
        if ($(filtro).val() == "") {
            alerta(lang_loadfilterko(),function () {
                ...
                success:function (response) {
                    ...
                    $(response["rows"]).each(function () {
                        ...
                        var interval = setInterval(function () {
                            ...
                            if (type == "checkbox") {
                                setCheck(temp[0],temp[1] ? true : false); // CHECKBOX FIELD
                            }
                            if (type != "checkbox") {
                                setParam(temp[0],temp[1]); // OTHER FIELD
                            }
                            if (substr(type,0,6) == "select") {
                                $("form[id=list] *[name=" + temp[0] + "]").attr("original",temp[1]);
                            }
    ...
    function update_filter()
    {
        var filtro = $("#id_filter");
        if ($(filtro).val() == "") {
            alerta(lang_updatefilterko(),function () {
                $(filtro).trigger("focus"); });
        } else {
    ...
    function delete_filter()
    {
        var filtro = $("#id_filter");
        if ($(filtro).val() == "") {
            alerta(lang_deletefilterko(),function () {
                $(filtro).trigger("focus"); });
        } else {
            loadingcontent(lang_sending());
            var id_filter = $(filtro).val();
    ...
    function create_filter()
    {
        ...
        if ($(filtro).val() == "") {
            alerta(lang_createfilterko(),function () {
                $(filtro).trigger("focus"); });
        } else {
            loadingcontent(lang_sending());
    ...
    function rename_filter()
    {
        ...
        if ($(filtro1).val() == "") {
            alerta(lang_renamefilterko1(),function () {
                $(filtro1).trigger("focus"); });
        } else if ($(filtro2).val() == "") {
            alerta(lang_renamefilterko2(),function () {
                $(filtro2).trigger("focus"); });
        } else {
            loadingcontent(lang_sending());
