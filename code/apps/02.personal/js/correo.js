...
if (typeof __correo__ == "undefined" && typeof parent.__correo__ == "undefined") {
    ...
    function update_state(id,type,value)
    {
        ...
        $.ajax({
            ...
            success:function (response) {
                ...
                if (type == "spam") {
                    if (value == 1) {
                        notice(lang_alert(),lang_msgnumsispam());
                    }
                    if (value == 0) {
                        notice(lang_alert(),lang_msgnumnospam());
                    }
                }
            },
    ...
    function update_files()
    {
        ...
        if (id_extra.length == 3 && id_extra[1] == "forward") {
            id = id_extra[2];
        }
        if (id_extra.length == 3 && id_extra[1] == "session") {
            id = id_extra[1];
        }
        if (!$("td[id$=files],label[id$=files]").length) {
            id = 0;
        }
        if (!id) {
            return;
        ...
        if ($(checkbox).prop("checked")) {
            var interval = setInterval(function () {
                if (!$(checkbox).length) {
                    ...
                    if (typeof __mobile__ != "undefined") {
