...
if (typeof __importaciones__ == "undefined" && typeof parent.__importaciones__ == "undefined") {
    ...
    function import_pager(info,page,pages,first,previous,next,last)
    {
        ...
        if (!first) {
            $("a[id$=firstpager]").addClass("ui-state-disabled");
        }
        if (!previous) {
            $("a[id$=previouspager]").addClass("ui-state-disabled");
        }
        if (!next) {
            $("a[id$=nextpager]").addClass("ui-state-disabled");
        }
        if (!last) {
            $("a[id$=lastpager]").addClass("ui-state-disabled");
        }
        if (first) {
            $("a[id$=firstpager]").removeClass("ui-state-disabled");
        }
        if (previous) {
            $("a[id$=previouspager]").removeClass("ui-state-disabled");
        }
        if (next) {
            $("a[id$=nextpager]").removeClass("ui-state-disabled");
        }
        if (last) {
            $("a[id$=lastpager]").removeClass("ui-state-disabled");
        }
    }
