...
if(typeof(__checkemail__)=="undefined" && typeof(parent.__checkemail__)=="undefined") {
	...
	function check_inbox(sync) {
		...
		if(sync) {
			...
			submit1(function () { inbox_executing=0; });
			return
		}
		// NORMAL USAGE
	...
	function check_outbox(sync) {
		...
		if(sync) {
			...
			submit1(function () { outbox_executing=0; });
			return
		}
		// NORMAL USAGE
		$.ajax({
	...
	$(document).ready(function() {
		if(config_inbox_interval()>0) {
			...
			setInterval(function() {
				inbox_counter=inbox_executing?0:inbox_counter+1000;
		...
		if(config_outbox_interval()>0) {
			...
			setInterval(function() {
				outbox_counter=outbox_executing?0:outbox_counter+1000;
				if(outbox_counter>=config_outbox_interval()) {
					...
					if(is_correo_list()) {
						var interval=setInterval(function() {
							if(!outbox_executing) {
								clearInterval(interval);
								if(disabled) $("#enviar").addClass("ui-state-disabled");
								if(!disabled) $("#enviar").removeClass("ui-state-disabled");
							}
						},1000);
					}
					outbox_counter=0;
