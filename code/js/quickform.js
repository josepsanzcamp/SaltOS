...
if(typeof(__quickform__)=="undefined" && typeof(parent.__quickform__)=="undefined") {
	...
	function update_quickform(id) {
		if(!$("#quick_select").length) return;
		...
		$.ajax({
			...
			success:function (response) {
				...
				$("#quick_select").html2(options);
				if($("root>first",response).text()) $("#quick_first").removeClass("ui-state-disabled");
				if($("root>previous",response).text()) $("#quick_previous").removeClass("ui-state-disabled");
				if($("root>next",response).text()) $("#quick_next").removeClass("ui-state-disabled");
				if($("root>last",response).text()) $("#quick_last").removeClass("ui-state-disabled");
			},
