...
saltos.add_header_button = function (option) {
    ...
    if (option.class2 == "right") {
        $("#navbar-right").append(button);
    } else if (option.class2 == "center") {
        $("#navbar-center").append(button);
    } else {
        $("#navbar-left").append(button);
    }
};
...
saltos.make_table = function (option) {
    ...
    var table = $(`
        <table class="tabla helperlists table table-striped table-hover table-sm" cellpadding="0" cellspacing="0" border="0">
            <thead></thead>
            <tbody></tbody>
        </table>
    `);
    ...
    if (option.checkbox) {
        $("thead tr",table).append(`
            <td class="width1 thead alert-primary">
    ...
    for (var key in option.fields) {
        ...
        var td = $(`<td class="thead alert-primary"></td>`);
    ...
    if (count(option.actions)) {
        $("thead tr",table).append(`
            <td class="width1 thead alert-primary" colspan="100">
                <span class=""></span></td>
        `);
    }
    // BODY
    ...
    for (var key in option.rows) {
        ...
        $("tbody tr:last td",table).addClass("border-bottom");
        total++;
    }
    if (!total) {
        $("tbody",table).append("<tr></tr>");
        $("tbody tr:last",table).append(`
            <td colspan="100" class="tbody nodata italic">
    ...
    if (found) {
        ...
        $("tbody",table).append("<tr></tr>");
        if (option.checkbox) {
            $("tbody tr:last",table).append(`<td class="width1 thead alert-primary"></td>`);
        }
        for (var key in math_row) {
            $("tbody tr:last",table).append(`<td class="thead alert-primary"></td>`);
            if (isset(math_row[key].label)) {
                $("tbody tr:last td:last",table).append(math_row[key].label);
            }
        }
        if (count(option.actions)) {
            $("tbody tr:last",table).append(`<td class="width1 thead alert-primary" colspan="100"></td>`);
        }
        // BODY
        $("tbody",table).append("<tr></tr>");
        if (option.checkbox) {
            $("tbody tr:last",table).append(`<td class="width1 tbody"></td>`);
        }
        for (var key in math_row) {
        ...
        if (count(option.actions)) {
            $("tbody tr:last",table).append(`<td class="width1 tbody" colspan="100"></td>`);
        }
        $("tbody tr:last td",table).addClass("notop border-bottom");
    }
    return table;
};
...
saltos.form_field_button = function (field) {
    ...
    var td = $(`
        <td colspan="${field.colspan}" rowspan="${field.rowspan}" class="${field.class}" style="width:${field.width}">
            <button focused="${field.focus}" labeled="${field.label}" style="width:${field.width2}"
                title="${field.tip}" class="btn btn-primary mr-1 ${field.class2}" id="${field.name}">
                <span class="${field.icon}"></span></button></td>
    `);
    $("button",td).append(" " + field.value);
    if (field.disabled == "true") {
        $("button",td).addClass("ui-state-disabled");
    } else {
        //~ $("button",td).on("click",{event:field.onclick},saltos.__form_event);
        $("button",td).attr("onclick",field.onclick);
    }
    obj.push(td);
    return obj;
};
...
$.fn.tabs = function (a,b,c) {
    console.log("Executing fn.tabs");
    console.log(a);
    console.log(b);
    console.log(c);
    if (a == "option" && b == "active") {
        $(".centertabs li:eq(" + c + ") a").tab("show");
    }
};
